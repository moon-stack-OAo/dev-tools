...
describe("parseBatch - 反向: date → ts(秒和毫秒)", () => {
  test("date → Unix 秒", () => {
    ...
    expect(r.ok[0].output).toBe(
      String(Math.floor(new Date(s).getTime() / 1000)),
    );
  });
...
describe("tsFormat - 三种输出格式", () => {
  ...
  test("yyyy-MM-dd HH:mm:ss(UTC)", () => {
    expect(tsFormat(ms, "yyyy-MM-dd HH:mm:ss", "UTC")).toBe(
      "2023-11-14 22:13:20",
    );
  });
...
describe("tsFormatInZone - 时区格式化", () => {
  ...
  test("Asia/Shanghai(UTC+8)", () => {
    expect(tsFormatInZone(new Date(ms), "Asia/Shanghai")).toBe(
      "2023-11-15 06:13:20",
    );
  });
});

...
describe("snowflakeId - 生成与位运算", () => {
  ...
  test("时间戳 ≥ 2^41 抛错", () => {
    expect(() => snowflakeId(0, 0, 0, SNOWFLAKE_EPOCH + (1n << 41n))).toThrow(
      /41/,
    );
  });
});
...
describe("nextLeafId - 美团 Leaf", () => {
  test("使用 LEAF_EPOCH (2020-01-01)", () => {
    ...
    const p = parseSnowflake(
      id.toString(),
      require("../../js/generate/snowflake.js").LEAF_EPOCH,
    );
...
describe("sfBase36ToBigInt - 进制辅助函数", () => {
  ...
  test("大小写均接受", () => {
    expect(sfBase36ToBigInt("FF").toString()).toBe(
      sfBase36ToBigInt("ff").toString(),
    );
  });
  test("非法字符抛错", () => {

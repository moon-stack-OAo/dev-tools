...
} = require("../../js/debug/stacktrace.js");
...
describe("formatStackTraceHtml - 输出", () => {
  test("空结果返回占位符", () => {
    ...
    expect(
      formatStackTraceHtml({ exceptions: [], currentException: null }),
    ).toMatch(/无法解析/);
  });
  test("异常类型与消息出现在 HTML 中", () => {
    const html = formatStackTraceHtml(
      parseStackTrace("java.lang.RuntimeException: <boom>"),
    );
  ...
  test("异常之间有分隔线 (border-top 仅出现在第二个及之后)", () => {
    const html = formatStackTraceHtml(parseStackTrace(SIMPLE));
    expect(html.match(/border-top:1px dashed/g)?.length).toBeGreaterThanOrEqual(
      1,
    );
  });
});

  ...
  test("sorted 字段升序", () => {
    const r = generateStreamCode({
      ...
      steps: [
        {
          kind: "sorted",
          comparator: { type: "field", field: "age", direction: "asc" },
        },
      ],
      terminal: { kind: "collect", collect: "toList" },
    });
  ...
  test("sorted 字段降序附加 reversed", () => {
    const r = generateStreamCode({
      ...
      steps: [
        {
          kind: "sorted",
          comparator: { type: "field", field: "name", direction: "desc" },
        },
      ],
      terminal: { kind: "collect", collect: "toList" },
    });
...
describe("generateStreamCode group/partition", () => {
  test("groupBy + downstream toList", () => {
    ...
    expect(r.code).toContain(
      "Collectors.groupingBy(User::getCity, Collectors.toList())",
    );

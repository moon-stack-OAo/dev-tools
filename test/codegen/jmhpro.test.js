...
describe("generateJmhClass @Group", () => {
  test("A+B 输出 Phaser + Scope.Group", () => {
    ...
    cfg.methods = [
      {
        ...
        returnType: "void",
        params: [],
        group: { name: "g1", groupThreads: 1, role: "A" },
      },
      {
        name: "consumer",
        returnType: "void",
        params: [],
        group: { name: "g1", groupThreads: 1, role: "B" },
      },
    ];
    ...
    expect(out).toMatch(
      /void consumer\(\)[\s\S]*long startNanos = phaser\.arrive\(\)/,
    );
  });
    ...
    cfg.methods = [
      {
        ...
        returnType: "void",
        params: [],
        group: { name: "g1", groupThreads: 3, role: "A" },
      },
      {
        name: "b",
        returnType: "void",
        params: [],
        group: { name: "g1", groupThreads: 3, role: "B" },
      },
    ];
    cfg.opts = { addMain: false, addSetup: false, addTearDown: false };
    const out = generateJmhClass("", "M", cfg.methods, cfg.global, cfg.opts);
    ...
    cfg.methods = [
      {
        name: "a",
        returnType: "void",
        params: [{ name: "size", values: ["100", "1000"] }],
        group: null,
      },
      {
        name: "b",
        returnType: "void",
        params: [{ name: "size", values: ["200", "2000"] }],
        group: null,
      },
    ];
    cfg.opts = { addMain: false, addSetup: false, addTearDown: false };
...
describe("generateJmhClass @Fork", () => {
  test("jvmArgs 多行(按换行分割)", () => {
    ...
    const out = generateJmhClass("", "M", cfg.methods, cfg.global, cfg.opts);
    expect(out).toContain(
      '@Fork(value = 2, jvmArgs = {"-Xms512m", "-Xmx1024m", "-XX:+UseG1GC"})',
    );
  });
});
...
describe("generateMethod 独立函数", () => {
  ...
  test("非 void 返回类型 return null/0/empty", () => {
    const a = generateMethod(
      { name: "foo", returnType: "String", params: [], group: null },
      "M",
    );
    expect(a).toContain('return "";');
    const b = generateMethod(
      { name: "bar", returnType: "long", params: [], group: null },
      "M",
    );
    expect(b).toContain("return 0;");
    const c = generateMethod(
      { name: "baz", returnType: "Object", params: [], group: null },
      "M",
    );
...
describe("basic 模式 - 单方法签名", () => {
  ...
  test('返回类型为 String 时返回 "" 占位', () => {
    const cfg = defaultConfig();
    cfg.methods = [
      { name: "bench", returnType: "String", params: [], group: null },
    ];
    const out = generateJmhClass("", "M", cfg.methods, cfg.global, cfg.opts);

...
function makeMockDom(values) {
  ...
  return {
    getElementById: (id) =>
      map[id] ||
      (id === "jmpBasicState" ? null : { value: "", checked: false }),
  };
}
...
describe("generateBasicJmh 自定义字段", () => {
  ...
  test("不同 BenchmarkMode 映射到 Mode.X", () => {
    expect(generateBasicJmh({ mode: "AverageTime" })).toContain(
      "@BenchmarkMode(Mode.AverageTime)",
    );
    expect(generateBasicJmh({ mode: "SampleTime" })).toContain(
      "@BenchmarkMode(Mode.SampleTime)",
    );
    expect(generateBasicJmh({ mode: "SingleShotTime" })).toContain(
      "@BenchmarkMode(Mode.SingleShotTime)",
    );
    expect(generateBasicJmh({ mode: "All" })).toContain(

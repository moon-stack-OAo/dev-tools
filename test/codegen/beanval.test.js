...
describe("parseJavaFields 基础解析", () => {
  test("解析单个 private 字段", () => {
    ...
    expect(r).toEqual([
      { name: "username", type: "String", typeRaw: "String" },
    ]);
  });
...
describe("fieldAnnoSpec 类型推断", () => {
  ...
  test("Integer → Min(0) + Max(默认)", () => {
    ...
    expect(r.find((a) => a.anno === "Max").params).toBe(
...
describe("inferValidations 必填与组合", () => {
  test("必填 + Integer → NotNull + Min + Max", () => {
    const r = inferValidations(
      [{ name: "count", type: "Integer", required: true, custom: "" }],
      {},
    );
  ...
  test("必填 String → NotBlank 已隐含 NotNull,不重复", () => {
    ...
    const notNullCount = r[0].annotations.filter(
      (a) => a.anno === "NotNull" || a.anno === "NotBlank",
    ).length;
  ...
  test("用户指定 dateType=Past 覆盖名称启发式", () => {
    const r = inferValidations(
      [{ name: "eventDay", type: "LocalDate", required: false, custom: "" }],
      {
        dateType: "Past",
      },
    );
  ...
  test("intMax 自定义生效", () => {
    const r = inferValidations(
      [{ name: "quantity", type: "Integer", required: false, custom: "" }],
      {
        intMax: 9999,
      },
    );
...
describe("collectImports", () => {
  test("收集并去重", () => {
    ...
    expect(
      r.filter((x) => x === "javax.validation.constraints.NotBlank"),
    ).toHaveLength(1);
  });
...
describe("generateBean 输出完整性", () => {
  ...
  test("getterSetter=false 时不生成 getter/setter", () => {
    const specs = inferValidations(
      [{ name: "a", type: "String", required: false, custom: "" }],
      {},
    );
...
describe("renderAnnotation / dedupAnnos", () => {
  ...
  test("renderAnnotation 带参数", () => {
    expect(renderAnnotation({ anno: "Size", params: "max = 10" })).toBe(
      "@Size(max = 10)",
    );
  });

...
describe('javaToPlantUml 输出', () => {
    ...
    test('基本类输出包含 @startuml/class/end', () => {
        const code = `class Foo {}`;
        const out = javaToPlantUml(code, fullOpts);
    ...
    test('showMethods=true 渲染 +method(...): Type 且参数带类型', () => {
        const code = `class Foo {
        ...
        expect(out).toContain('+setX(x: int): void');
    });
    ...
    test('默认开启 skin 头', () => {
        const out = javaToPlantUml('class Foo {}', {
            showFields: true,
            showMethods: true,
            showConstructors: true,
            showModifiers: true,
        ...
        expect(out).toContain('skinparam classAttributeIconSize 0');
        expect(out).toContain('skinparam monochrome false');
        expect(out).toContain('hide empty members');
    });

    test('skin: false 关闭皮肤', () => {
        const out = javaToPlantUml('class Foo {}', { skin: false });
        expect(out).not.toContain('skinparam classAttributeIconSize');
        expect(out).not.toContain('hide empty members');
    });

    test('direction lr 输出 left to right', () => {
        const out = javaToPlantUml('class Foo {}', { skin: false, direction: 'lr' });
        expect(out).toContain('left to right direction');
    });

    test('showAssociations 生成字段关联', () => {
        const code = `
            class Order { private Customer customer; private List<Item> items; }
        ...
        expect(out).toContain('Order --> Customer : customer');
        expect(out).toContain('Order --> Item : items');
        expect(out).not.toMatch(/Order --> String/);
    });

    test('showAssociations=false 不生成字段关联', () => {
        const code = `
        ...
        expect(out).not.toContain('Order --> Customer');
    });

    test('package 声明生成 package 块', () => {
        const code = `package com.demo; class Foo {}`;
        const out = javaToPlantUml(code, { skin: false, showPackage: true });
        expect(out).toContain('package com.demo {');
        expect(out).toContain('class Foo');
    });

    test('showPackage=false 不输出 package', () => {
        const code = `package com.demo; class Foo {}`;
        const out = javaToPlantUml(code, { skin: false, showPackage: false });
        expect(out).not.toContain('package com.demo');
    });
});
...
describe('jsonToPlantUml 输出', () => {
    ...
    test('单对象生成 Root 类 + 字段', () => {
        const out = jsonToPlantUml('{"a":1,"b":"x","c":true}', {
            showFields: true,
            showMethods: true,
            skin: false,
        });
    ...
    test('嵌套对象生成嵌套类 + 带字段名关系', () => {
        const out = jsonToPlantUml('{"addr":{"city":"上海"}}', { skin: false });
        expect(out).toContain('class Root');
        expect(out).toContain('class Address');
        expect(out).toContain('Root "1" *-- "0..*" Address : addr');
    });

    test('数组根生成 Root1/Root2', () => {
        const out = jsonToPlantUml('[{"x":1},{"y":2}]', { skin: false });
    ...
    test('非 JSON 输入不抛错返回空图或 note', () => {
        const out = jsonToPlantUml('not json', { skin: false });
        expect(out).toContain('@startuml');
        expect(out).toContain('@enduml');
    });

    test('数组项为对象时字段类型为 List<Item> 且关系带字段名', () => {
        const out = jsonToPlantUml('{"items":[{"x":1}]}', { skin: false });
        expect(out).toContain('+items: List<Item>');
        expect(out).toContain('class Item');
        expect(out).toContain('Root "1" *-- "0..*" Item : items');
    });

    test('opts.rootName 覆盖根类名', () => {
        const out = jsonToPlantUml('{"a":1}', { skin: false, rootName: 'User' });
        expect(out).toContain('class User');
        expect(out).toContain('+a: Integer');
    });

    test('默认 skin 头存在', () => {
        const out = jsonToPlantUml('{"a":1}', {});
        expect(out).toContain('skinparam classAttributeIconSize 0');
        expect(out).toContain('hide empty members');
    });
});
...
describe('plantumlEncodeSync 编码', () => {
    ...
    test('中文 case 与 encode 联调', () => {
        ...
        const uml = javaToPlantUml(code, { skin: false });

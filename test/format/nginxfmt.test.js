...
describe("formatNginx 基本指令", () => {
  ...
  test("缩进控制 - 4 空格（默认）", () => {
    const out = formatNginx("http { server { listen 80; } }", {
      indent: "    ",
    });
    expect(out).toBe("http {\n    server {\n        listen 80;\n    }\n}");
  });
  ...
  test("多参数 directive", () => {
    const out = formatNginx("  listen  80  default_server;", {
      indent: "    ",
    });
...
describe("formatNginx 块嵌套", () => {
  ...
  test("upstream 含多条 server", () => {
    const src =
      "http { upstream b { server 127.0.0.1:80; server 127.0.0.1:81; } }";
    const out = formatNginx(src, { indent: "    " });
...
describe("formatNginx location 正则", () => {
  test("location ~ 正则含点号转义", () => {
    const src =
      "server { location ~ \\.php$ { fastcgi_pass unix:/tmp/php.sock; } }";
    const out = formatNginx(src, { indent: "    " });

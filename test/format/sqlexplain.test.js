...
describe("parsePgExplain (textual)", () => {
  test("3 节点树(1+2)按缩进构造", () => {
    ...
    expect(tree[0].children[1].nodeType).toBe(
      "Index Only Scan using idx_user on orders orders",
    );
...
describe("formatMysqlTabularTable", () => {
  test("输出 <table> + thead/tbody 结构", () => {
    const html = formatMysqlTabularTable(
      parseMysqlExplain(SAMPLE_MYSQL_TABULAR),
    );

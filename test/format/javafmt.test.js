...
describe("formatJava 基本缩进", () => {
  ...
  test("4 空格缩进 嵌套 if", () => {
    const out = formatJava("class A{void m(){if(true){return;}}}", {
      indent: "    ",
    });
...
describe("formatJava 嵌套与复杂", () => {
  ...
  test("if-else 链", () => {
    const input =
      "class A{void m(int x){if(x==1){return;}else if(x==2){return;}else{return;}}}";
    const out = formatJava(input, { indent: "    " });
...
describe("formatJava import 排序分组", () => {
  test("按 java/javax/其他 分组并字母序", () => {
    ...
    // 字母序
    expect(lines.indexOf("import java.util.List;")).toBeLessThan(
      lines.indexOf("import java.util.Map;"),
    );
  });
...
describe("formatJava 注解换行", () => {
  test("一行 3 个注解 拆成 3 行", () => {
    const out = formatJava(
      '@Override @Deprecated @SuppressWarnings("all") public void m(){}',
      {
        indent: "    ",

...
describe("parseDDL 单表", () => {
  test("基本字段解析（3 个列）", () => {
    const tables = parseDDL(
      "CREATE TABLE users (id INT NOT NULL, name VARCHAR(50), email VARCHAR(100));",
    );
  ...
  test("COMMENT 解析", () => {
    const tables = parseDDL(
      "CREATE TABLE t (name VARCHAR(50) COMMENT '用户名');",
    );
  ...
  test("COMMENT 含转义引号", () => {
    const tables = parseDDL(
      "CREATE TABLE t (name VARCHAR(50) COMMENT 'it''s ok');",
    );
  ...
  test("字符串内含括号 / 逗号不影响解析", () => {
    const tables = parseDDL(
      "CREATE TABLE t (id INT NOT NULL, note VARCHAR(100) DEFAULT 'a,b (c)');",
    );
    expect(tables[0].columns.length).toBe(2);
  ...
  test("列级 PRIMARY KEY / UNIQUE 保留在字段上", () => {
    const tables = parseDDL(
      "CREATE TABLE t (id INT PRIMARY KEY, code VARCHAR(10) UNIQUE);",
    );
...
describe("diffSchemas", () => {
  ...
  test("default 变化", () => {
    ...
    const dst = parseDDL(
      "CREATE TABLE t (status VARCHAR(10) DEFAULT 'inactive');",
    );
    const diff = diffSchemas(src, dst);
    const changed = diff[0].changes.find((c) => c.action === "changed");
    expect(changed).toBeDefined();
  ...
  test("comment 变化", () => {
    ...
    const dst = parseDDL(
      "CREATE TABLE t (name VARCHAR(50) COMMENT 'user name');",
    );
    const diff = diffSchemas(src, dst);
    const changed = diff[0].changes.find((c) => c.action === "changed");
    expect(changed).toBeDefined();
  ...
  test("多表分别 diff", () => {
    const src = parseDDL(
      "CREATE TABLE a (id INT); CREATE TABLE b (id INT, name VARCHAR(50));",
    );
    const dst = parseDDL(
      "CREATE TABLE a (id INT, new_col INT); CREATE TABLE b (id INT);",
    );
    const diff = diffSchemas(src, dst);
    ...
    expect(
      a.changes.some((c) => c.action === "added" && c.column === "new_col"),
    ).toBe(true);
    expect(
      b.changes.some((c) => c.action === "removed" && c.column === "name"),
    ).toBe(true);
  });
  ...
  test("完全相同的 schema 无变化", () => {
    const ddl =
      "CREATE TABLE t (id INT NOT NULL, name VARCHAR(50), age INT DEFAULT 0);";
  ...
  test("changed 行同时含多个 diff 字段", () => {
    const src = parseDDL(
      "CREATE TABLE t (id INT NOT NULL DEFAULT 0 COMMENT 'old');",
    );

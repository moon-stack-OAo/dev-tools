...
describe('expandRanges', () => {
    ...
    test('非法 totalPages 抛错', () => {
        ...
        expect(() => expandRanges([[1, 3]], 1.5)).toThrow('totalPages 必须为正整数');
    });
...
describe('mergePdfs', () => {
    ...
    test('非 Uint8Array 抛错', async () => {
        await expect(mergePdfs(['not bytes'])).rejects.toThrow('不是有效的 PDF 字节数据');
        await expect(mergePdfs([new ArrayBuffer(10)])).rejects.toThrow('不是有效的 PDF 字节数据');
    });

    test('pdf-lib 未加载抛错', async () => {
        global.window = {};
        await expect(mergePdfs([new Uint8Array(10)])).rejects.toThrow('pdf-lib 库未加载');
    });
...
describe('splitPdf', () => {
    test('非法字节抛错', async () => {
        ...
        await expect(splitPdf('not bytes', [[1, 1]])).rejects.toThrow('PDF 字节数据无效');
    });

    test('空 ranges 抛错', async () => {
        await expect(splitPdf(new Uint8Array(10), [])).rejects.toThrow('页码范围为空');
        await expect(splitPdf(new Uint8Array(10), null)).rejects.toThrow('页码范围为空');
    });

    test('pdf-lib 未加载抛错', async () => {
        global.window = {};
        await expect(splitPdf(new Uint8Array(10), [[1, 1]])).rejects.toThrow('pdf-lib 库未加载');
    });
    ...
    test('所有页码越界抛错', async () => {
        await expect(splitPdf(new Uint8Array([1]), [[100, 200]])).rejects.toThrow('没有有效的页码落在文档范围内');
    });
});
...
describe('splitPdfMultiple', () => {
    test('空 ranges 抛错', async () => {
        await expect(splitPdfMultiple(new Uint8Array(10), [])).rejects.toThrow('页码范围为空');
    });
...
describe('getPageCount', () => {
    ...
    test('pdf-lib 未加载抛错', async () => {
        global.window = {};
        await expect(getPageCount(new Uint8Array(10))).rejects.toThrow('pdf-lib 库未加载');
    });
});

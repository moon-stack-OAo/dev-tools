const MAVEN_CMDS = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        cmd: "-Dmaven.compiler.source=17 -Dmaven.compiler.target=17",
        desc: "指定 Java 编译版本",
      },
  ...
  },
  {
    ...
    items: [
      ...
      {
        cmd: "mvn spring-boot:repackage",
        desc: "将 jar 重新打包为可执行 fat jar",
      },
      {
        cmd: "mvn spring-boot:build-image",
        desc: "使用 buildpacks 构建 Docker 镜像（Spring Boot 2.3+）",
      },
      { cmd: "mvn mybatis-generator:generate", desc: "运行 MyBatis Generator" },
      {
        cmd: "mvn jib:build",
        desc: "使用 Jib 直接构建 Docker 镜像（无需 docker daemon）",
      },
  ...
  },
  {
    ...
    items: [
      { cmd: "mvn help:system", desc: "查看 Java / Maven / 系统环境信息" },
      {
        ...
        desc: "合并后的 effective POM（含继承 / profile 合并）",
      },
      {
        cmd: "mvn help:effective-settings",
        desc: "合并后的 effective settings.xml",
      },
      { cmd: "mvn help:active-profiles", desc: "当前激活的 profile" },
      { cmd: "mvn help:describe -Dplugin=compiler", desc: "查看插件信息" },
      {
        cmd: "mvn help:evaluate -Dexpression=project.basedir",
        desc: "求值 Maven 表达式",
      },
      { cmd: "mvn archetype:generate", desc: "从 archetype 生成项目骨架" },
      {
        cmd: "mvn archetype:create-from-project",
        desc: "从现有项目生成 archetype",
      },
    ],
  },
  {

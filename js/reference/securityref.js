const SECURITY_REFS = [
    {
        ...
        items: [
            ...
            },
            {
                ...
                desc: '方法参数过滤',
                examples: ['@PreFilter("filterObject.owner == authentication.name")\npublic void updateDocs(List<Document> docs) {}'],
                returns: '参数过滤'
            },
            {
                ...
                desc: '返回值过滤',
                examples: ['@PostFilter("filterObject.owner == authentication.name")\npublic List<Document> getDocs() {}'],
                returns: '返回值过滤'
            }
        ]
    },
    ...
    },
    {
        ...
        items: [
            ...
            },
            {
                ...
                examples: ['.oauth2ResourceServer(oauth2 -> oauth2\n    .jwt(jwt -> jwt.jwtAuthenticationConverter(converter)))'],
                returns: 'JWT 资源服务器'
            },
            {
                ...
                desc: 'OAuth2 登录',
                examples: ['.oauth2Login(oauth2 -> oauth2\n    .loginPage("/login")\n    .defaultSuccessUrl("/dashboard"))'],
                returns: 'OAuth2 登录配置'
            },
            {
                ...
                desc: '记住我功能',
                examples: ['.rememberMe(remember -> remember\n    .key("uniqueKey")\n    .tokenValiditySeconds(86400))'],
                returns: '记住我配置'
            },
            {
                ...
                desc: '会话管理',
                examples: ['.sessionManagement(session -> session\n    .maximumSessions(1)\n    .maxSessionsPreventsLogin(false))'],
                returns: '会话管理配置'
            },
            {
            ...
            },
            {
                ...
                desc: 'CORS 配置',
                examples: ['.cors(cors -> cors.configurationSource(request -> {\n    CorsConfiguration config = new CorsConfiguration();\n    config.setAllowedOrigins(List.of("*"));\n    return config;\n}))'],
                returns: 'CORS 配置'
            },
            {
        ...
        items: [
            {
                ...
                desc: 'BCrypt 哈希编码器',
                examples: ['PasswordEncoder encoder = new BCryptPasswordEncoder();\nString hash = encoder.encode("password");\nboolean matches = encoder.matches("password", hash);'],
                returns: 'BCrypt 编码器'
            },
            {
                ...
                desc: 'SCrypt 哈希编码器',
                examples: ['PasswordEncoder encoder = new SCryptPasswordEncoder();\nString hash = encoder.encode("password");'],
                returns: 'SCrypt 编码器'
            },
            {
                ...
                desc: 'Argon2 哈希编码器',
                examples: ['PasswordEncoder encoder = new Argon2PasswordEncoder();\nString hash = encoder.encode("password");'],
                returns: 'Argon2 编码器'
            },
            {
                ...
                desc: 'PBKDF2 哈希编码器',
                examples: ['PasswordEncoder encoder = new Pbkdf2PasswordEncoder();\nString hash = encoder.encode("password");'],
                returns: 'PBKDF2 编码器'
            },
            {
                ...
                examples: ['PasswordEncoder encoder = PasswordEncoderFactories.createDelegatingPasswordEncoder();\nString hash = encoder.encode("password"); // {bcrypt}$2a$10$...'],
                returns: '委托编码器'
            }
        ]
    },
    {
        ...
        items: [
            ...
            },
            {
                ...
                desc: '构建用户',
                examples: ['UserDetails user = User.builder()\n    .username("user")\n    .password(passwordEncoder.encode("pass"))\n    .roles("USER")\n    .build();'],
                returns: 'UserDetails'
            },
            {
    ...
    },
    {
        ...
        items: [
            ...
            },
            {
                ...
                desc: 'JWT 认证转换器',
                examples: ['JwtAuthenticationConverter converter = new JwtAuthenticationConverter();\nconverter.setJwtGrantedAuthoritiesConverter(new JwtGrantedAuthoritiesConverter());'],
            ...
            },
            {
                ...
                desc: 'JWK Set URI',
                examples: ['spring.security.oauth2.resourceserver.jwt.jwk-set-uri=https://auth.example.com/.well-known/jwks.json'],
                returns: 'JWK 配置'
            }
        ]
    },
    {
        ...
        items: [
            ...
            },
            {
                ...
                desc: '自定义权限检查',
                examples: ['@PreAuthorize("@securityService.hasPermission(#id, \'READ\')")\npublic Document getDocument(Long id) {}'],
                returns: '自定义权限检查'
            },
            {
            ...
            },
            {
                ...
                syntax: '@PostAuthorize("<spel>")',
                desc: '返回值状态检查',
                examples: ['@PostAuthorize("returnObject.status == T(Status).ACTIVE")\npublic Order getOrder(Long id) {}'],
                returns: '返回值检查'
            }
        ]
    },
...
function securityrefEscapeHtml(s) {
...
function securityrefRender(filter) {
    ...
    SECURITY_REFS.forEach(group => {
        ...
        section.innerHTML = `<div style="font-size:12px;font-weight:600;color:var(--accent);padding:6px 0;border-bottom:1px solid var(--border);margin-bottom:8px">${securityrefEscapeHtml(group.cat)}</div>`;
        matched.forEach(item => {
            const card = document.createElement('div');
            card.className = 'ref-card';
            let html = `<div class="ref-cmd-head"><code class="ref-cmd-name">${securityrefEscapeHtml(item.cmd)}</code><span class="ref-cmd-desc">${securityrefEscapeHtml(item.desc)}</span><button class="sm outline" onclick="safeCopy('${securityrefEscapeHtml(item.cmd).replace(/'/g, "\\'")}')">复制</button></div>`;
            if (item.syntax && item.syntax !== item.cmd) {
                html += `<div class="ref-syntax">${securityrefEscapeHtml(item.syntax)}</div>`;
            }
            if (item.examples && item.examples.length) {
                html += `<div class="ref-section-title">示例</div>`;
                item.examples.forEach(ex => {
                    html += `<div class="ref-copy-wrap"><pre class="ref-pre"><code>${securityrefEscapeHtml(ex)}</code></pre><button class="ref-copy-btn" onclick="securityrefCopyPre(this, event)">复制</button></div>`;
                });
            }
            card.innerHTML = html;

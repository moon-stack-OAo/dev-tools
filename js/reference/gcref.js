...
function gcrefRender(filter) {
    ...
    GC_REFS.forEach(group => {
        ...
        section.innerHTML = `<div style="font-size:12px;font-weight:600;color:var(--accent);padding:6px 0;border-bottom:1px solid var(--border);margin-bottom:8px">${gcrefEscapeHtml(group.cat)}</div>`;
        matched.forEach(item => {
            const card = document.createElement('div');
            card.className = 'ref-card';
            let html = `<div class="ref-cmd-head"><code class="ref-cmd-name">${gcrefEscapeHtml(item.cmd)}</code><span class="ref-cmd-desc">${gcrefEscapeHtml(item.desc)}</span><button class="sm outline" onclick="safeCopy('${gcrefEscapeHtml(item.cmd).replace(/'/g, "\\'")}')">复制</button></div>`;
            if (item.syntax && item.syntax !== item.cmd) {
                html += `<div class="ref-syntax">${gcrefEscapeHtml(item.syntax)}</div>`;
            }
            if (item.examples && item.examples.length) {
                html += `<div class="ref-section-title">示例</div>`;
                item.examples.forEach(ex => {
                    html += `<div class="ref-copy-wrap"><pre class="ref-pre"><code>${gcrefEscapeHtml(ex)}</code></pre><button class="ref-copy-btn" onclick="gcrefCopyPre(this, event)">复制</button></div>`;
                });
            }
            if (item.returns) {
                html += `<div style="font-size:11px;color:var(--text-muted);margin-top:6px"><strong>输出:</strong> ${gcrefEscapeHtml(item.returns)}</div>`;
            }
            card.innerHTML = html;

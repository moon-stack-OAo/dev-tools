    ...
    },
    {
        ...
        items: [
            ...
            },
            {
                ...
                desc: '验证方法调用',
                examples: ['verify(userRepository, times(1)).save(any());\nverify(userRepository, never()).delete(any());'],
                returns: '验证结果'
            },
            {
                cmd: 'doReturn(...).when(...)',
            ...
            },
            {
                ...
                desc: '捕获方法参数',
                examples: ['ArgumentCaptor<User> captor = ArgumentCaptor.forClass(User.class);\nverify(userRepository).save(captor.capture());\nUser savedUser = captor.getValue();'],
                returns: '参数捕获器'
            }
        ]
    },
...
function junit5Render(filter) {
    ...
    JUNIT5_REFS.forEach(group => {
        ...
        section.innerHTML = `<div style="font-size:12px;font-weight:600;color:var(--accent);padding:6px 0;border-bottom:1px solid var(--border);margin-bottom:8px">${junit5EscapeHtml(group.cat)}</div>`;
        matched.forEach(item => {
            const card = document.createElement('div');
            card.className = 'ref-card';
            let html = `<div class="ref-cmd-head"><code class="ref-cmd-name">${junit5EscapeHtml(item.cmd)}</code><span class="ref-cmd-desc">${junit5EscapeHtml(item.desc)}</span><button class="sm outline" onclick="safeCopy('${junit5EscapeHtml(item.cmd).replace(/'/g, "\\'")}')">复制</button></div>`;
            if (item.syntax && item.syntax !== item.cmd) {
                html += `<div class="ref-syntax">${junit5EscapeHtml(item.syntax)}</div>`;
            }
            if (item.examples && item.examples.length) {
                html += `<div class="ref-section-title">示例</div>`;
                item.examples.forEach(ex => {
                    html += `<div class="ref-copy-wrap"><pre class="ref-pre"><code>${junit5EscapeHtml(ex)}</code></pre><button class="ref-copy-btn" onclick="junit5CopyPre(this, event)">复制</button></div>`;
                });
            }
            card.innerHTML = html;

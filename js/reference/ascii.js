...
function asciiRender() {
  ...
  tbl.style.cssText =
    "font-family:var(--font);font-size:12px;overflow:auto;max-height:calc(100vh - 200px)";
  let html =
  ...
  ASCII_DATA.forEach((item) => {
    ...
    html +=
      '<span style="width:40px;padding:2px 8px;color:var(--accent2)">' +
      item.hex +
      "</span>";
    html +=
      '<span style="width:50px;padding:2px 8px;color:var(--text-muted)">' +
      item.oct +
      "</span>";
    html +=
      '<span style="width:80px;padding:2px 8px;' +
      (isCtrl ? "color:var(--text-muted)" : "") +
      '">' +
      (item.char || "•") +
      "</span>";
    html +=
      '<span style="flex:1;padding:2px 8px;color:var(--text-dim)">' +
      item.name +
      "</span>";
    html += "</div>";
  });

    ...
    items: [
      {
        ...
        example:
          "Group A 包含 C1/C2/C3 三个 Consumer 共同消费 Topic-X 的 6 个分区",
      },
      {
        key: "再均衡 (Rebalance)",
        type: "概念",
      ...
      },
      {
        ...
        example:
          "__consumer_offsets 50 个分区，分区键 hash(groupId + topic + partition)",
      },
      {
...
const RABBITMQ_REF = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        example:
          "order.* 匹配 order.created；order.# 匹配 order.created.refund",
      },
      {
        key: "fanout",
        type: "类型",
...
function mqrefRender(filter) {
  ...
  data.forEach((group) => {
    ...
    const matched = filter
      ? group.items.filter((it) => mqrefItemMatches(it, filter))
      : group.items;

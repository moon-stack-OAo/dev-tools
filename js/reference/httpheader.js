const HTTP_HEADERS = [
    // 通用
    {name: 'Cache-Control', dir: '通用', example: 'no-cache, max-age=3600', desc: '缓存控制：no-cache / no-store / max-age / public / private'},
    {name: 'Connection', dir: '通用', example: 'keep-alive | close', desc: '连接管理：keep-alive 长连接 / close 短连接（HTTP/1.1 默认 keep-alive）'},
    ...
    {name: 'Transfer-Encoding', dir: '通用', example: 'chunked', desc: '传输编码：chunked 分块传输（响应体长度未知时使用）'},
    ...
    {name: 'Warning', dir: '通用', example: '110 - "Response is Stale"', desc: '代理对消息状态的警告（已基本被弃用）'},

    // 请求
    {name: 'Accept', dir: '请求', example: 'application/json, text/html', desc: '客户端可接受的响应内容类型（支持 q 权重）'},
    ...
    {name: 'Accept-Language', dir: '请求', example: 'zh-CN,zh;q=0.9,en;q=0.8', desc: '客户端可接受的语言（用于国际化）'},
    {name: 'Authorization', dir: '请求', example: 'Bearer eyJhbGciOi...', desc: '认证凭证：Basic / Bearer token / Digest 等'},
    {name: 'Cookie', dir: '请求', example: 'sid=abc123; theme=dark', desc: '客户端发送的 Cookie'},
    {name: 'Host', dir: '请求', example: 'api.example.com:8080', desc: '目标主机（HTTP/1.1 必填，区分同一 IP 多域名）'},
    {name: 'If-Match', dir: '请求', example: '"67ab43"', desc: '条件请求：ETag 匹配才执行（乐观锁 / 防止并发修改）'},
    {name: 'If-Modified-Since', dir: '请求', example: 'Thu, 01 Jan 2025 00:00:00 GMT', desc: '条件请求：资源在指定时间后修改才返回'},
    ...
    {name: 'X-Forwarded-For', dir: '请求', example: '203.0.113.1, 10.0.0.1', desc: '客户端真实 IP（代理层追加，逗号分隔）'},
    ...
    // 响应
    {name: 'Access-Control-Allow-Origin', dir: '响应', example: 'https://example.com', desc: 'CORS 允许的来源（* 或具体域）'},
    {name: 'Access-Control-Allow-Methods', dir: '响应', example: 'GET, POST, PUT, DELETE', desc: 'CORS 允许的 HTTP 方法'},
    {name: 'Access-Control-Allow-Headers', dir: '响应', example: 'Content-Type, Authorization', desc: 'CORS 允许的请求头'},
    ...
    {name: 'Content-Disposition', dir: '响应', example: 'attachment; filename="a.pdf"', desc: '内容处置：inline 直接显示 / attachment 下载'},
    ...
    {name: 'Location', dir: '响应', example: 'https://example.com/new', desc: '重定向目标 URL（301 / 302 / 201 / 3xx）'},
    {name: 'Server', dir: '响应', example: 'nginx/1.25.0', desc: '服务器软件信息（建议隐藏或模糊化）'},
    {name: 'Set-Cookie', dir: '响应', example: 'sid=abc; Path=/; HttpOnly; Secure', desc: '设置 Cookie（可指定 Path / Domain / Max-Age / HttpOnly / Secure / SameSite）'},
    {name: 'Strict-Transport-Security', dir: '响应', example: 'max-age=31536000; includeSubDomains', desc: 'HSTS：强制 HTTPS（首次 HTTPS 响应后生效）'},
    {name: 'X-Content-Type-Options', dir: '响应', example: 'nosniff', desc: '禁止浏览器 MIME 嗅探'},
    {name: 'X-Frame-Options', dir: '响应', example: 'DENY | SAMEORIGIN', desc: '是否允许在 <iframe> 中展示（防点击劫持）'},
    {name: 'X-XSS-Protection', dir: '响应', example: '1; mode=block', desc: '浏览器 XSS 过滤（已基本弃用，由 CSP 替代）'},
    {name: 'Content-Security-Policy', dir: '响应', example: "default-src 'self'", desc: 'CSP：限制资源加载来源（防 XSS）'},
...
function httpheaderRender(filter) {
    ...
    header.style.cssText = 'display:grid;grid-template-columns:240px 60px 240px 1fr;background:var(--bg-input);font-weight:600;border-radius:4px;padding:6px 0;margin-bottom:6px;font-size:12px;position:sticky;top:0';
    header.innerHTML = `
    ...
    list.forEach(h => {
        ...
        row.style.cssText = 'display:grid;grid-template-columns:240px 60px 240px 1fr;padding:6px 0;border-bottom:1px solid var(--border);font-size:12px;cursor:pointer;transition:background .12s;border-radius:4px';

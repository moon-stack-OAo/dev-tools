...
function designpatternsRender(filter) {
  ...
  DESIGN_PATTERNS.forEach((group) => {
    ...
    matched.forEach((item) => {
      ...
      card.style.cssText =
        "margin-bottom:8px;border:1px solid var(--border);border-radius:6px;overflow:hidden";
      ...
      headerHtml +=
        '<i class="bi bi-chevron-down" style="color:var(--text-dim);transition:transform .2s"></i>';
      header.innerHTML = headerHtml;
      const codeBlock = document.createElement("div");
      codeBlock.style.cssText =
        "display:none;padding:12px;background:var(--bg-input);position:relative";
      let codeHtml = "";
      ...
      header.addEventListener("click", function () {
        ...
        header.querySelector("i").style.transform = isOpen
          ? ""
          : "rotate(180deg)";
      });
  ...
  if (!hasResult) {
    container.innerHTML =
      '<div style="color:var(--text-dim);padding:20px;text-align:center">无匹配结果</div>';
  }
}

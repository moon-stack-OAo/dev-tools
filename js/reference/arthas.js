    ...
    },
    {
        ...
        items: [
            ...
            },
            {
                ...
                examples: [
                    'dump demo.MathGame',
                    'dump -d /tmp/dump demo.MathGame',
                    'dump demo.MathGame -c 3be2cf6b'
                ],
                outputs: [
        ...
        items: [
            {
                ...
                examples: [
                    'monitor demo.MathGame primeFactors -n 10',
        ...
        items: [
            ...
            },
            {
                ...
                examples: [
                    ...
                    'mbean -m java.lang:type=Memory HeapMemoryUsage'
                ],
                outputs: [
...
function arthasBuildExampleBlock(label, content, kind) {
    const safe = arthasEscapeHtml(content);
...
function arthasRender(filter) {
    ...
    arthasCommands.forEach(group => {
        ...
        section.innerHTML = `<div style="font-size:12px;font-weight:600;color:var(--accent);padding:6px 0;border-bottom:1px solid var(--border);margin-bottom:8px">${arthasEscapeHtml(group.cat)}</div>`;
        matched.forEach(item => {
            ...
            let html = `
                <div class="arthas-cmd-head">
                    <code class="arthas-cmd-name">${arthasEscapeHtml(item.cmd)}</code>
                    <span class="arthas-cmd-desc">${arthasEscapeHtml(item.desc)}</span>
                    <button class="sm outline" onclick="arthasCopyCmd('${arthasEscapeHtml(item.cmd).replace(/'/g, "\\'")}')">复制命令</button>
                </div>
            `;

            if (item.scenario) {
                html += `<div class="arthas-scenario">${arthasEscapeHtml(item.scenario)}</div>`;
            }

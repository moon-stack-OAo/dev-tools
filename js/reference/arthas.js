const arthasCommands = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        scenario:
          "OOM/内存泄漏排查时导出 hprof 文件离线分析（MAT、VisualVM）。",
        examples: [
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        desc: "内存模型信息及使用情况",
        scenario:
          "排查 OOM 时查看各分区使用情况（heap/metaspace/code cache）。",
        examples: ["memory"],
        outputs: [
...
function arthasItemMatches(item, filter) {
  ...
  if (
    item.examples &&
    item.examples.some((e) => e.toLowerCase().includes(filter))
  )
    return true;
  if (
    item.outputs &&
    item.outputs.some((o) => o.toLowerCase().includes(filter))
  )
    return true;
  return false;
}
...
function arthasRender(filter) {
  ...
  arthasCommands.forEach((group) => {
    const matched = filter
      ? group.items.filter((i) => arthasItemMatches(i, filter))
      : group.items;

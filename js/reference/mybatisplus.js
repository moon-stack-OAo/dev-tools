const MYBATIS_PLUS_DATA = [
  {
    ...
    items: [
      ...
      {
        method: "deleteByMap(Map<String, Object>)",
        desc: "根据 columnMap 条件删除",
      },
      { method: "delete(Wrapper<T> wrapper)", desc: "根据条件构造器删除" },
      {
        method: "deleteBatchIds(Collection<? extends Serializable>)",
        desc: "批量删除（按 ID 集合）",
      },
      {
        method: "updateById(T entity)",
        desc: "根据 ID 更新（NULL 字段不更新）",
      },
      ...
      {
        method: "selectList(Wrapper<T> wrapper)",
        desc: "查询列表（满足条件全部）",
      },
      {
        method: "selectOne(Wrapper<T> wrapper)",
        desc: "查询单条（结果 >1 会抛异常）",
      },
      { method: "selectCount(Wrapper<T> wrapper)", desc: "查询总数" },
      {
        method: "selectMaps(Wrapper<T> wrapper)",
        desc: "返回 List<Map<String, Object>>",
      },
      {
        method: "selectObjs(Wrapper<T> wrapper)",
        desc: "返回 List<Object>（只取第一列）",
      },
      {
        method: "selectPage(IPage<T> page, Wrapper<T>)",
        desc: "分页查询，返回 IPage<T>",
      },
      {
        method: "selectMapsPage(IPage, Wrapper)",
        desc: "分页查询，返回 IPage<Map>",
      },
    ],
  },
  {
    ...
    items: [
      ...
      {
        method: "saveBatch(Collection<T> list)",
        desc: "批量保存（默认 1000/批）",
      },
      {
        method: "saveBatch(Collection<T> list, int batchSize)",
        desc: "指定批次大小批量保存",
      },
      ...
      {
        method: "removeByIds(Collection<? extends Serializable>)",
        desc: "按 ID 集合批量删除",
      },
      { method: "remove(Wrapper<T> wrapper)", desc: "按条件删除" },
      { method: "updateById(T entity)", desc: "按 ID 更新" },
      {
        method: "update(Wrapper<T> updateWrapper)",
        desc: "按条件更新（entity 不可为 null）",
      },
      ...
      {
        method: "chain()",
        desc: "链式调用，返回 ChainQuery / LambdaQueryChainWrapper",
      },
    ],
  },
  {
    ...
    items: [
      ...
      {
        method: "between(SFunction, val1, val2)",
        desc: "BETWEEN val1 AND val2",
      },
      ...
      {
        method: "select(Class<T>, Predicate<TableFieldInfo>)",
        desc: "过滤字段查询",
      },
      { method: "or() / or(Consumer<LambdaQueryWrapper>)", desc: "OR 拼接" },
      { method: "and(Consumer<LambdaQueryWrapper>)", desc: "AND 嵌套" },
      {
        method: "nested(Consumer<LambdaQueryWrapper>)",
        desc: "嵌套括号 ( ... )",
      },
      {
        method: "apply(String applySql, Object... params)",
        desc: "拼接 SQL 片段（参数自动绑定）",
      },
  ...
  },
  {
    ...
    items: [
      ...
      {
        method: "incr(SFunction, long val)",
        desc: "原子自增（set field = field + val）",
      },
      { method: "decr(SFunction, long val)", desc: "原子自减" },
      {
        method: "eq / ne / gt / lt ...",
        desc: "同 LambdaQueryWrapper 的条件方法",
      },
    ],
  },
  {
    ...
    items: [
      {
        method: "new Page<T>(long current, long size)",
        desc: "构造分页，current=1 起",
      },
      {
        method: "new Page<T>(current, size, total)",
        desc: "已知总数时构造分页",
      },
      {
        method: "new Page<T>(current, size, total, boolean searchCount)",
        desc: "可关闭 count 查询",
      },
      { method: "IPage.setRecords(List<T>)", desc: "设置数据列表" },
      {
        method: "IPage.setCurrent(long) / setSize(long)",
        desc: "修改当前页/页大小",
      },
      { method: "IPage.setTotal(long) / getTotal()", desc: "总数" },
      {
        method: "IPage.setSearchCount(boolean)",
        desc: "关闭 count SQL（已知总数时提速）",
      },
  ...
  },
  {
    ...
    items: [
      {
        method: "Wrappers.lambdaQuery()",
        desc: "构造 LambdaQueryWrapper（链式起点）",
      },
      { method: "Wrappers.lambdaUpdate()", desc: "构造 LambdaUpdateWrapper" },
      {
        method: "Wrappers.query() / Wrappers.update()",
        desc: "非 Lambda 构造器",
      },
      {
        method: "QueryWrapper<T>.lambda()",
        desc: "QueryWrapper 转 LambdaQueryWrapper",
      },
    ],
  },
];

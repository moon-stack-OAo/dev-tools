  ...
  welcome: {
    ...
    fields: [
      ...
      },
      {
        ...
        default:
          "感谢您注册 {{company}}，您的账户已激活。立即点击下方按钮开始体验完整功能。",
      },
    ],
    build: (v, t) =>
      baseLayout(
  ...
  order: {
    ...
    fields: [
      ...
      },
      {
        key: "orderNo",
        label: "订单号",
        placeholder: "202506160001",
        default: "202506160001",
      },
      {
        key: "orderDate",
        label: "订单日期",
        placeholder: "2025-06-16 10:30",
        default: "2025-06-16 10:30",
      },
      {
        key: "amount",
        label: "订单金额",
  ...
  event: {
    ...
    fields: [
      ...
      },
      {
        key: "eventTitle",
        label: "活动标题",
        placeholder: "2025 开发者大会",
        default: "2025 开发者大会",
      },
      {
        key: "eventTime",
        label: "活动时间",
        placeholder: "2025-07-20 14:00",
        default: "2025-07-20 14:00",
      },
      {
        key: "eventLocation",
        label: "活动地点",
      ...
      },
      {
        ...
        placeholder: "活动详情...",
        default:
          "本次活动汇聚业内顶尖技术专家，分享最新技术趋势与最佳实践，现场还有精美礼品与抽奖环节。",
      },
    ],
    build: (v, t) =>
      baseLayout(
  ...
  promo: {
    ...
    fields: [
      ...
      },
      {
        ...
        placeholder: "尊享专属优惠...",
        default:
          "为感谢您的支持，我们为您准备了一份专属优惠。点击下方按钮立即使用优惠码，享受限时折扣！",
      },
    ],
    build: (v, t) =>
      baseLayout(
...
function emailInlineCss(html) {
  ...
  html = html.replace(/<(\w+)([^>]*)>/g, (match, tag, attrs) => {
    if (
      ["br", "hr", "img", "meta", "link", "title", "span"].indexOf(
        tag.toLowerCase(),
      ) >= 0 &&
    ...
    if (styleMatch) {
      styleMatch[1].split(";").forEach((d) => {
        const ci = d.indexOf(":");
        if (ci > 0)
          inline[d.substring(0, ci).trim().toLowerCase()] = d
            .substring(ci + 1)
            .trim();
      });
    }
...
function emailCurrentTheme() {
  return {
    primaryColor:
      document.getElementById("emailPrimary").value ||
      DEFAULT_THEME.primaryColor,
    bgColor: document.getElementById("emailBg").value || DEFAULT_THEME.bgColor,
    cardColor:
      document.getElementById("emailCard").value || DEFAULT_THEME.cardColor,
    textColor:
      document.getElementById("emailText").value || DEFAULT_THEME.textColor,
    mutedColor:
      document.getElementById("emailMuted").value || DEFAULT_THEME.mutedColor,
    borderColor:
      document.getElementById("emailBorder").value || DEFAULT_THEME.borderColor,
    btnTextColor:
      document.getElementById("emailBtnText").value ||
      DEFAULT_THEME.btnTextColor,
    footerText:
      document.getElementById("emailFooterText").value ||
      DEFAULT_THEME.footerText,
    companyName:
      document.getElementById("emailCompany").value ||
      DEFAULT_THEME.companyName,
    unsubscribeUrl:
      document.getElementById("emailUnsub").value ||
      DEFAULT_THEME.unsubscribeUrl,
...
function emailRenderInlined() {
  ...
  document.getElementById("emailSize").textContent = formatBytes(
    inlined.length,
  );
...
function emailCopyInlined() {
  ...
  safeCopy(
    inlined,
    "已复制内联 CSS 的 HTML (" + formatBytes(inlined.length) + ")",
  );
}
...
function emailInit() {
  ...
  if (!sel.options.length) {
    Object.keys(EMAIL_TEMPLATES).forEach((id) => {
      ...
      opt.value = id;
      opt.textContent =
        EMAIL_TEMPLATES[id].icon + " " + EMAIL_TEMPLATES[id].name;
      sel.appendChild(opt);
    });
  }

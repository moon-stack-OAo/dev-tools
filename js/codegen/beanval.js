...
function fieldAnnoSpec(type, name, opts) {
  ...
  const o = opts || {};
  const intMax =
    typeof o.intMax === "number" ? o.intMax : Number.MAX_SAFE_INTEGER;
  ...
  switch (t) {
    case "String": {
      if (onStr) {
        list.push({
          anno: "NotBlank",
          params: "",
          import: ANNO_IMPORT.NotBlank,
        });
        list.push({
          anno: "Size",
          params: "max = 255",
          import: ANNO_IMPORT.Size,
        });
      }
      if (onHeu) {
    ...
    case "Long": {
      if (onNum) {
        ...
        if (onHeu && /(^|_)age(_|$)/.test(n)) {
          ...
          list.push({
            anno: "Max",
            params: String(intMax),
            import: ANNO_IMPORT.Max,
          });
        }
      }
      break;
    ...
    case "BigDecimal": {
      if (onDec) {
        list.push({
          anno: "Digits",
          params: "integer = 10, fraction = 2",
          import: ANNO_IMPORT.Digits,
        });
      }
      break;
    }
...
function inferValidations(fields, opts) {
  ...
  for (const f of fields || []) {
    ...
    if (requiredNotNull && f.required && type !== "Boolean") {
      const hasNot = annos.some(
        (a) => a.anno === "NotBlank" || a.anno === "NotNull",
      );
      if (!hasNot) {
        annos.unshift({
          anno: "NotNull",
          params: "",
          import: ANNO_IMPORT.NotNull,
        });
      }
    }
...
function generateBean(packageName, className, fieldSpecs, opts) {
  ...
  if (o.validated)
    extras.push("org.springframework.validation.annotation.Validated");
...
function bvReadOpts() {
  return {
    ...
    intMax:
      Number(document.getElementById("bvOptIntMax").value) ||
...
function bvAddField(prefill) {
  ...
  const selType = bvBuildTypeSelect(
    prefill && prefill.type ? prefill.type : "String",
  );
...
function bvDoImport() {
  ...
  if (!parsed.length) {
    if (msg) {
      ...
      msg.textContent =
        "未解析到字段,请检查格式 (形如: private String username;)";
    }
    return;
  }
  parsed.forEach((p) => {

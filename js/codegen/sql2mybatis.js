...
function sql2mbParseDDL(ddl) {
  ...
  for (const rawRow of rows) {
    ...
    if (
      /^(COMMENT|ENGINE|DEFAULT\s+CHARSET|DEFAULT\s+CHARSET\s*=|CHARSET\s*=)/i.test(
        line,
      )
    ) {
      continue;
    ...
    const typeMatch = restAfterName.match(
      /^([A-Za-z]+)\s*(\(\s*([0-9]+)\s*\))?/,
    );
    ...
    const typeInfo = SQL2MB_TYPE_MAP[sqlType] || {
      java: "String",
      jdbc: "VARCHAR",
    };
    ...
    const defaultMatch = line.match(
      /DEFAULT\s+('(?:[^'\\]|\\.|'')*'|NULL|CURRENT_TIMESTAMP|NOW\(\)|[0-9.+-]+)/i,
    );
...
function sql2mbAutoParse() {
  ...
  if (!entityEl.value.trim())
    entityEl.value = sql2mbInferEntityName(parsed.table);
...
function sql2mbGenerate() {
  ...
  const table = (
    document.getElementById("sql2mbTable").value || parsed.table
  ).trim();
  const entity = (
    document.getElementById("sql2mbEntity").value ||
    sql2mbInferEntityName(parsed.table)
  ).trim();
  ...
  const namespace = (
    document.getElementById("sql2mbNamespace").value ||
    "com.example.mapper." + entity + "Mapper"
  ).trim();
...
function sql2mbBuildXml(cfg) {
  ...
  lines.push("    <!-- 主键查询 -->");
  lines.push(
    '    <select id="selectByPrimaryKey" parameterType="' +
      pkJava +
      '" ' +
      sql2mbResultMapAttr(cfg) +
      ">",
  );
  lines.push("        select");
  ...
  lines.push("    <!-- 主键删除 -->");
  lines.push(
    '    <delete id="deleteByPrimaryKey" parameterType="' + pkJava + '">',
  );
  ...
  lines.push(
    "        values (" +
      cfg.fields
        .map(
          (f) =>
            "#{" + sql2mbToCamel(f.name) + ", jdbcType=" + f.jdbcType + "}",
        )
        .join(", ") +
      ")",
  );
  ...
  cfg.fields
    ...
    .forEach((f) => {
      lines.push(
        '            <if test="' + sql2mbToCamel(f.name) + ' != null">',
      );
      lines.push(
        "                `" +
          f.name +
  ...
  lines.push("        </set>");
  lines.push(
    "        where `" + cfg.pk + "` = #{" + sql2mbToCamel(cfg.pk) + "}",
  );
  lines.push("    </update>");
  lines.push("");
...
function sql2mbBuildMapperInterface(cfg) {
  ...
  lines.push("     */");
  lines.push(
    "    int deleteByPrimaryKey(" + sql2mbParam(cfg, pkJava, cfg.pk) + ");",
  );
  lines.push("");
  ...
  lines.push("     */");
  lines.push(
    "    int insertSelective(" + sql2mbParam(cfg, cfg.entity, "record") + ");",
  );
  lines.push("");
  ...
  lines.push(
    "    int updateByPrimaryKeySelective(" +
      sql2mbParam(cfg, cfg.entity, "record") +
      ");",
  );
  lines.push("");
...
function sql2mbCopyAll() {
  ...
  const parts = [];
  if (xmlText && !xmlText.startsWith("DDL"))
    parts.push("// ===== Mapper XML =====\n" + xmlText);
  if (javaText && !javaText.startsWith("DDL"))
    parts.push("// ===== Mapper Interface =====\n" + javaText);
  if (!parts.length) {
    toast("暂无可复制内容，请先生成");
    return;
  }
...
function sql2mbDownloadZip() {
  ...
  const hasXml =
    xmlText && !xmlText.startsWith("DDL") && !xmlText.startsWith("请输入");
  const hasJava =
    javaText && !javaText.startsWith("DDL") && !javaText.startsWith("请输入");
  if (!hasXml && !hasJava) {
    toast("请先生成");
    return;
  }
  ...
  const entityName =
    entityEl && entityEl.value.trim() ? entityEl.value.trim() : "User";
  const namespace =
    nsEl && nsEl.value.trim()
      ? nsEl.value.trim()
  ...
  const mapperPackage =
    lastDot > 0 ? namespace.substring(0, lastDot) : "com.example.mapper";
  ...
  const interfaceSimpleName =
    lastDot > 0 ? namespace.substring(lastDot + 1) : entityName + "Mapper";

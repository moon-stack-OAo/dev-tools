...
function parseJavaToClasses(code) {
  ...
  for (const h of headers) {
    ...
    classes.push({
      name: h.name,
      type:
        h.modifier && h.modifier.indexOf("abstract") >= 0 ? "abstract" : h.type,
      fields: fields,
      methods: methods,
...
function parseJsonToClasses(json) {
  ...
  function walk(obj, nameHint) {
    ...
    const className =
      nameHint && /^[A-Z]\w*$/.test(nameHint) ? nameHint : "Object";
  ...
  arr.forEach((root, idx) => {
    ...
    const rootName =
      arr.length === 1 && !Array.isArray(json) ? "Object" : "Root" + (idx + 1);
    walk(root, rootName);
  });
  return {
...
function _plRenderClass(cls, opts) {
  ...
  if (cls.type === "enum") {
    const seen = new Set();
    const consts = (cls.fields || []).filter((f) =>
  ...
  if (showMethods) {
    for (const m of cls.methods || []) {
      ...
      if (!m.isConstructor && m.name === cls.name && showCtors === false)
        continue;
      ...
      const ret = m.isConstructor
        ? ""
        : ": " + _plEscapeType(m.returnType || "void");
...
function plantumlEncode(text) {
  ...
  return new Response(stream)
    .arrayBuffer()
    .then((buf) => _plB64UrlSafe(new Uint8Array(buf)));
}

...
function defaultConfig() {
  return {
    ...
    methods: [
      { name: "baseline", returnType: "void", params: [], group: null },
    ],
    global: {
      mode: ["Throughput"],
...
function validateConfig(config) {
  ...
  if (g.outputTimeUnit && TIME_UNITS.indexOf(g.outputTimeUnit) < 0) {
    errs.push({
      field: "global.outputTimeUnit",
      msg: "未知 TimeUnit: " + g.outputTimeUnit,
    });
  }
  if (g.stateScope && STATE_SCOPES.indexOf(g.stateScope) < 0) {
    errs.push({
      field: "global.stateScope",
      msg: "未知 Scope: " + g.stateScope,
    });
  }
...
function _jmpRenderHeader(global) {
  ...
  if (mode.indexOf("All") >= 0 && mode.length === 1) {
    ...
    ["Throughput", "AverageTime", "SampleTime", "SingleShotTime"].forEach(
      (m) => {
        out.push("//   - Mode." + m);
      },
    );
    ...
    out.push(
      "@BenchmarkMode({ " + mode.map((m) => "Mode." + m).join(", ") + " })",
    );
  }
  if (g.compilerControl) {
    out.push(
      "@CompilerControl(CompilerControl.Mode." + g.compilerControl + ")",
    );
  }
  out.push(
    "@OutputTimeUnit(TimeUnit." + (g.outputTimeUnit || "MILLISECONDS") + ")",
  );
  ...
  if (args.length) {
    out.push(
      "@Fork(value = " + fv + ', jvmArgs = {"' + args.join('", "') + '"})',
    );
  } else {
    out.push("@Fork(" + fv + ")");
  }
  ...
  const wbs =
    warm.batchSize && warm.batchSize !== 1
      ? ", batchSize = " + warm.batchSize
      : "";
  out.push(
    "@Warmup(iterations = " +
      wi +
  ...
  const mbs =
    meas.batchSize && meas.batchSize !== 1
      ? ", batchSize = " + meas.batchSize
      : "";
  out.push(
...
function generateMethod(methodDef, stateClass) {
  ...
  const group = m.group;
  const paramNames = Array.isArray(m.params)
    ? m.params.map((p) => p.name).filter(Boolean)
    : [];
  let sig = "    @Benchmark\n";
  if (group) {
  ...
  if (addSetup) {
    if (usePhaser) {
      ...
      for (const m of safeMethods) {
        if (m.group)
          sz +=
            m.group.groupThreads && m.group.groupThreads !== 1
              ? m.group.groupThreads
              : 1;
      }
...
function _jmpReadGlobal() {
  ...
  return {
    ...
    outputTimeUnit:
      document.getElementById("jmpOutTime").value || "MILLISECONDS",
...
function jmpBindMethodsEvents() {
  ...
  container.addEventListener("input", (e) => {
    ...
    else if (t.classList.contains("jmp-m-ret"))
      _jmpMethods[i].returnType = t.value;
    else if (t.classList.contains("jmp-p-name") && _jmpMethods[i].params[pi])
      _jmpMethods[i].params[pi].name = t.value;
    else if (
      t.classList.contains("jmp-p-values") &&
      _jmpMethods[i].params[pi]
    ) {
      _jmpMethods[i].params[pi].values = t.value
        .split(/[\r\n,]/)
        .map((s) => s.trim())
        .filter(Boolean);
    } else if (t.classList.contains("jmp-m-gname") && _jmpMethods[i].group)
      _jmpMethods[i].group.name = t.value;
  ...
  container.addEventListener("change", (e) => {
    ...
    if (t.classList.contains("jmp-m-groupenabled")) {
      _jmpMethods[i].group = t.checked
        ? { name: "g1", groupThreads: 1, role: "A" }
        : null;
      jmpRenderMethods();
...
function jmpGenerate() {
  ...
  cfg.className =
    (document.getElementById("jmpClass").value || "").trim() || "MyBenchmark";
...
function jmpLoadConfig(cfg) {
  ...
  document.getElementById("jmpOutTime").value =
    cfg.global.outputTimeUnit || "MILLISECONDS";
  ...
  document.getElementById("jmpJvmArgs").value =
    (cfg.global.fork || {}).jvmArgs || "";
  ...
  document.getElementById("jmpStateScope").value =
    cfg.global.stateScope || "Thread";
  ...
  document.getElementById("jmpAddTearDown").checked =
    cfg.opts.addTearDown !== false;
...
function jmpGenerateBasic() {
  const out =
    document.getElementById("jmpBasicOutput") ||
    document.getElementById("jmpOutput");
  if (!out) return;
...
function jmpInit() {
  ...
  const tabAdvanced = document.getElementById("jmpTabAdvanced");
  if (tabBasic)
    tabBasic.addEventListener("click", () => switchJmpMode("basic"));
  if (tabAdvanced)
    tabAdvanced.addEventListener("click", () => switchJmpMode("advanced"));

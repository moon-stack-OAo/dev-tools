...
const STEP_DEFS = [
  {
    ...
    label: "filter",
    needsExpr: true,
    exprPlaceholder: "u -> u.getAge() > 18",
  },
  {
    kind: "map",
    label: "map",
    needsExpr: true,
    exprPlaceholder: "User::getName",
  },
  {
    kind: "sorted",
    label: "sorted",
  ...
  { kind: "distinct", label: "distinct" },
  {
    ...
    label: "peek",
    needsExpr: true,
    exprPlaceholder: "System.out::println",
  },
  {
    kind: "flatMap",
    label: "flatMap",
    needsExpr: true,
    exprPlaceholder: "u -> u.getOrders()",
  },
  {
    kind: "groupBy",
    label: "groupBy",
  ...
  switch (step.kind) {
    ...
    case "sorted": {
      ...
      if (
        !c ||
        c === "naturalOrder" ||
        (typeof c === "object" && c.type === "naturalOrder")
      ) {
        return ".sorted()";
      }
    ...
    case "groupBy": {
      ...
      return (
        ".collect(Collectors.groupingBy(" + getter + ", " + downstream + "))"
      );
    }
    case "partitionBy": {
      imports.add("java.util.stream.Collectors");
...
function jsReadTerminal() {
  ...
  if (def.kind === "collect") {
    ...
    if (def.needsKv) {
      out.key =
        (document.getElementById("jsTk").value || "").trim() || "k -> k";
      out.value =
        (document.getElementById("jsTv").value || "").trim() || "v -> v";
    }
    if (def.needsSep) {
    ...
    out.expr =
      (document.getElementById("jsTexpr").value || "").trim() || "x -> true";
  } else if (def.needsReduce) {
    out.identity = document.getElementById("jsTid").value;
    out.op =
      (document.getElementById("jsTop").value || "").trim() || "(a, b) -> a";
  }
  return out;
}
...
function jsReadSteps() {
  return _jsSteps.map((s) => {
    if (s.kind === "limit" || s.kind === "skip")
      return { kind: s.kind, n: parseInt(s.n, 10) || 0 };
    if (
      s.kind === "filter" ||
      s.kind === "map" ||
      s.kind === "peek" ||
    ...
    if (s.kind === "sorted")
      return { kind: "sorted", comparator: s.comparator };
...
function jsRenderTerminal() {
  const sel = document.getElementById("jsTerminal");
  sel.innerHTML = TERMINAL_DEFS.map(
...
function jsRenderTerminalExtra() {
  ...
  if (def.needsKv) {
    ...
    html =
      '<input id="jsTsep" placeholder="分隔符" value=", " style="width:120px">';
  } else if (def.needsExpr) {
    html =
      '<input id="jsTexpr" placeholder="lambda" value="x -> true" style="flex:1;min-width:200px">';
  } else if (def.needsReduce) {
    html =
...
function jsGenerate() {
  ...
  if (importsEl) {
    importsEl.textContent = result.imports.length
      ? result.imports.join("\n")
      : "// (无新增 import)";
  }
...
function jsAddSample() {
  _jsSteps = [
    ...
    {
      kind: "sorted",
      comparator: { type: "field", field: "name", direction: "asc" },
    },
  ];
  const tSel = document.getElementById("jsTerminal");
  const idx = TERMINAL_DEFS.findIndex(
    (t) => t.kind === "collect" && t.collect === "toList",
  );
    ...
    stepsEl.addEventListener("change", (e) => {
      ...
      if (t.classList.contains("js-kind")) {
        ...
        if (def.needsExpr) _jsSteps[i] = { kind: def.kind, expr: "" };
          ...
          _jsSteps[i] = {
            kind: "sorted",
            comparator: { type: "naturalOrder" },
          };
        else if (def.needsGroup)
          _jsSteps[i] = { kind: "groupBy", key: "", downstream: "toList" };

...
function imgbase64SwitchTab(tab) {
  ...
  contents.forEach((c) =>
    c.classList.toggle("active", c.id === "imgbase64Tab-" + tab),
  );
}
...
function img2b64RenderList() {
  ...
  if (!img2b64Files.length) {
    list.innerHTML =
      '<div style="font-size:12px;color:var(--text-muted);padding:8px 4px">（暂无文件）</div>';
    out.value = "";
    return;
  }
  list.innerHTML = "";
...
function img2b64Remove(i) {
  ...
  if (img2b64ActiveIdx >= img2b64Files.length)
    img2b64ActiveIdx = img2b64Files.length - 1;
  img2b64RenderList();
}
...
function img2b64HandleFiles(files) {
  ...
  Promise.all(tasks)
    .then((results) => {
      ...
      img2b64RenderList();
      setStatus(
        `已加载 ${results.length} 个文件，共 ${img2b64Files.length} 个`,
      );
    })
    .catch((err) => {
...
function b642imgRender() {
  ...
  if (!input.trim()) {
    preview.innerHTML =
      '<div class="placeholder">（请先输入 Base64 内容）</div>';
    meta.textContent = "";
  ...
  if (!data) {
    preview.innerHTML =
      '<div class="placeholder" style="color:var(--danger)">（Base64 内容为空）</div>';
    meta.textContent = "";
    b642imgCurrentUrl = null;
    return;
  }
  ...
  img.onerror = () => {
    preview.innerHTML =
      '<div class="placeholder" style="color:var(--danger)">（无效的 Base64 图片数据）</div>';
    meta.innerHTML =
      '<span style="color:var(--danger)">渲染失败，请检查内容</span>';
    b642imgCurrentUrl = null;
  };
  img.src = url;
...
function b642imgClear() {
  ...
  document.getElementById("b642imgPreview").innerHTML =
    '<div class="placeholder">（请先输入 Base64 内容）</div>';

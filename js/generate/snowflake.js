...
function sfGenerate(count) {
  ...
  const workerId =
    parseInt(document.getElementById("sfWorkerId").value, 10) || 0;
  const datacenterId =
    parseInt(document.getElementById("sfDcId").value, 10) || 0;
  ...
  const newItems = [];
  const startCount = list.querySelector("[data-empty]")
    ? 0
  ...
  newItems.reverse().forEach((item) => {
    ...
    if (
      list.firstChild &&
      list.firstChild.dataset &&
      list.firstChild.dataset.empty
    ) {
      list.innerHTML = "";
    }
...
function sfExportCSV() {
  ...
  const header =
    "Algorithm,ID,Timestamp(ms),Datetime(ISO),LocalTime,DatacenterID,WorkerID,Sequence\n";

...
function icRender() {
  ...
  const compSum = icItems.reduce(
    (s, it) => s + (it.outBlob ? it.outBlob.size : 0),
    0,
  );
  const ratio =
    origSum > 0 && compSum > 0 ? Math.round((1 - compSum / origSum) * 100) : 0;
  ...
  icItems.forEach((it, idx) => {
    ...
    const ratioNum =
      it.outBlob && it.file.size
        ? (1 - it.outBlob.size / it.file.size) * 100
        : 0;
    const ratioStr =
      it.outBlob && it.file.size ? ratioNum.toFixed(1) + "%" : "-";
    const status =
      it.status === "done"
        ? "✓ 已完成"
        : it.status === "processing"
          ...
          : it.status === "error"
            ...
            : "○ 待处理";
    const statusCls =
      it.status === "done" ? "ok" : it.status === "error" ? "err" : "pending";
...
function icUpdateQualityLabel() {
  document.getElementById("icQualityVal").textContent =
    document.getElementById("icQuality").value;
}
...
function icCompressInit() {
  ...
  document
    .getElementById("icQuality")
    .addEventListener("input", icUpdateQualityLabel);
  document
    .getElementById("icFormat")
    .addEventListener("change", icOnFormatChange);
}

...
function dateCalcAdd() {
    ...
    if (!dateStr) { out.textContent = '请选择基准日期'; return; }
    const d = new Date(dateStr);
    if (isNaN(d.getTime())) { out.textContent = '无效日期'; return; }
...
function dateCalcDiff() {
    ...
    if (!d1 || !d2) { out.textContent = '请选择两个日期'; return; }
    const a = new Date(d1), b = new Date(d2);
    if (isNaN(a.getTime()) || isNaN(b.getTime())) { out.textContent = '无效日期'; return; }
    const ms = Math.abs(b - a);
    const map = { 'day': 86400000, 'week': 604800000, 'month': 2592000000, 'year': 31536000000 };
    const val = (map[unit] || 86400000);
    out.textContent = Math.round(ms / val) + ' ' + ({ 'day': '天', 'week': '周', 'month': '月', 'year': '年' }[unit] || '天');
}

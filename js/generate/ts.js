...
function tsSwitchMode(mode) {
  ...
  document
    .getElementById("tsModeSingle")
    .classList.toggle("active", mode === "single");
  document
    .getElementById("tsModeBatch")
...
function tsRunSingle() {
  ...
  if (!tsRaw && !dateRaw) {
    out.innerHTML =
      '<div class="ts-result-empty">输入时间戳或日期后,这里显示 5 种格式结果</div>';
    return;
  }
  let ms = null;
  let errMsg = null;
  ...
  const rows = [
    { key: "UTC", icon: "bi-globe", val: tsFormatInZone(d, "UTC") },
    {
      key: "上海",
      icon: "bi-geo-alt",
      val: tsFormatInZone(d, "Asia/Shanghai"),
    },
...
function tsRunBatch() {
  ...
  if (summary) {
    ...
    summary.innerHTML =
      ...
      " 成功</span>" +
      (errNum
        ? ' <span class="ts-summary-divider">/</span> <span class="ts-summary-fail">' +
          errNum +
          " 失败</span>"
        : "");
  }
  setStatus(
    "解析完成: " + result.ok.length + " 成功 / " + result.err.length + " 失败",
  );
}
...
function tsInit() {
  ...
  if (tsDateInput)
    tsDateInput.addEventListener("input", debounce(tsRunSingle, 200));

  // 批量模式
  const batchInput = document.getElementById("tsBatchInput");
  if (batchInput)
    batchInput.addEventListener("input", debounce(tsRunBatch, 200));
...
function tsShowDetected(input) {
  ...
  if (!s) {
    ...
    badge.innerHTML =
      '<i class="bi bi-check-circle-fill"></i>识别为 Unix 秒(10 位)';
  } else if (/^-?\d{13}$/.test(s)) {
    badge.classList.add("ts-detected-ms");
    badge.innerHTML =
      '<i class="bi bi-check-circle-fill"></i>识别为 Unix 毫秒(13 位)';
  } else if (/^-?\d+(\.\d+)?$/.test(s)) {
    const n = Number(s);
    if (isFinite(n)) {
      if (Math.abs(n) < 1e12) {
      ...
      badge.innerHTML =
        '<i class="bi bi-exclamation-triangle-fill"></i>无法识别';
    }
  } else {
    badge.classList.add("ts-detected-err");

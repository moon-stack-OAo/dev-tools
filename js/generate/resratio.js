...
function resratioMatchStandard(w, h) {
  ...
  for (const r of RESRATIO_STANDARD) {
    ...
    const inv = r.h / r.w;
    if (Math.abs(f - inv) < 0.01)
      return { w: r.h, h: r.w, name: r.name + "（纵向）" };
  }
  return null;
}
...
function resratioCompute() {
  ...
  const parts = [];
  parts.push(
    '<div style="font-size:24px;font-weight:600;color:#22c55e;margin-bottom:4px">' +
      rw +
      " : " +
      rh +
      "</div>",
  ...
  parts.push(
    '<div style="color:var(--text-dim);font-size:12px;margin-bottom:10px">最简整数比例</div>',
  );
  ...
  if (matched) {
    ...
    parts.push(
      '<div style="margin-top:6px;color:var(--text-dim)">非标准比例</div>',
    );
  }
  parts.push(
    '<div style="margin-top:8px">总像素数：<b>' +
      totalFmt +
      "</b> ≈ " +
      mp +
      " MP</div>",
...
function resByRatioChange() {
  ...
  document.getElementById("resByCustomBox").style.display =
    v === "custom" ? "" : "none";
}

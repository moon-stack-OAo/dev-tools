...
function _cpInitPkijs() {
  ...
  if (
    typeof pki.setEngine === "function" &&
    typeof crypto !== "undefined" &&
    crypto.subtle
  ) {
    try {
      pki.setEngine("webcrypto", {
        name: "webcrypto",
        crypto: crypto,
...
function _cpParseOneCertificate(der) {
  ...
  if (asn1.result && !asn1.result.verified) {
    try {
      Object.defineProperty(asn1.result, "verified", {
        value: true,
        writable: true,
        configurable: true,
      });
...
function _cpFormatRdn(rdn, sep) {
  ...
  for (const tv of rdn.typesAndValues) {
    ...
    if (Array.isArray(val))
      val = val.map((v) => String.fromCharCode(v)).join("");
    map[key] = String(val);
  }
...
function _cpFormatSerial(cert) {
  ...
  const hexView =
    cert.serialNumber.valueBlock && cert.serialNumber.valueBlock.valueHexView;
    ...
    switch (n.type) {
      ...
      case 7: {
        ...
        if (Array.isArray(n.value) || n.value instanceof Uint8Array) {
          const arr =
            n.value instanceof Uint8Array ? n.value : new Uint8Array(n.value);
          if (arr.length === 4)
            out.ip.push(arr[0] + "." + arr[1] + "." + arr[2] + "." + arr[3]);
...
function _cpExtractKeyUsage(ext) {
  ...
  const hexView =
    ext.parsedValue.valueBlock && ext.parsedValue.valueBlock.valueHexView;
  if (!hexView) return [];
...
function _cpExtractBasicConstraints(extensions) {
  ...
  return {
    ...
    pathLen:
      ext.parsedValue.pathLenConstraint != null
...
function parseAllCertificates(text) {
  ...
  for (let i = 0; i < input.blocks.length; i++) {
    ...
    out.push({
      ...
      keyUsage: _cpExtractKeyUsage(
        extensions.find((e) => e.extnID === "2.5.29.15"),
      ),
      extKeyUsage: _cpExtractExtKeyUsage(
        extensions.find((e) => e.extnID === "2.5.29.37"),
      ),
...
function _cpRenderOneCert(c) {
  ...
  if (
    ...
    if (c.san.dns.length)
      rows.push(
        '<div class="certparser-san-row"><b>DNS:</b> ' +
          c.san.dns.map(escapeHtml).join(", ") +
          "</div>",
      );
    if (c.san.ip.length)
      rows.push(
        '<div class="certparser-san-row"><b>IP:</b> ' +
          c.san.ip.map(escapeHtml).join(", ") +
          "</div>",
      );
    if (c.san.uri.length)
      rows.push(
        '<div class="certparser-san-row"><b>URI:</b> ' +
          c.san.uri.map(escapeHtml).join(", ") +
          "</div>",
      );
    if (c.san.email.length)
      rows.push(
        '<div class="certparser-san-row"><b>Email:</b> ' +
          c.san.email.map(escapeHtml).join(", ") +
          "</div>",
      );
    sanHtml =
  ...
  if (c.crl.length) {
    crlHtml =
      ...
      "</a>" +
      (c.crl.length > 1
        ? ' <span class="certparser-dim">(+' + (c.crl.length - 1) + ")</span>"
        : "") +
      "</span></div>";
  }
  let aiaHtml = "";
  ...
  if (c.basicConstraints) {
    const t =
      ...
      (c.basicConstraints.pathLen != null
        ? " · pathLen: " + c.basicConstraints.pathLen
        : "");
    bcHtml =
...
function certparserParse() {
  ...
  try {
    ...
    document.getElementById("certparserResults").innerHTML =
      '<div class="certparser-empty">解析失败</div>';
...
function certparserClear() {
  ...
  _cpResults = [];
  document.getElementById("certparserResults").innerHTML =
    '<div class="certparser-empty">解析结果将在这里显示</div>';
  setStatus("已清空");
}

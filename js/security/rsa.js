...
async function rsaEncrypt() {
    ...
    if (!input) { out.textContent = '请输入明文'; return; }
    try {
        let key;
        if (pubKey && rsaKeyPair) {
            ...
            out.textContent = '请先生成密钥对或粘贴公钥'; return;
        }
...
async function rsaDecrypt() {
    ...
    if (!input) { out.textContent = '请输入密文'; return; }
    try {
        let key;
        if (privKey && rsaKeyPair) {
            ...
            out.textContent = '请先生成密钥对或粘贴私钥'; return;
        }
    ...
    } catch (e) { out.textContent = '解密失败: ' + e.message; }
}

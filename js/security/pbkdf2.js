...
function _pbkdf2ReadConfig() {
  ...
  const saltLen =
    parseInt(document.getElementById("pbkdf2SaltLen").value, 10) || 16;
  const iterations =
    parseInt(document.getElementById("pbkdf2Iterations").value, 10) || 100000;
  const dkLen =
    parseInt(document.getElementById("pbkdf2DkLen").value, 10) || 32;
  return {
    algo: algo,
    password: password,
    saltLen: saltLen,
...
async function pbkdf2Generate() {
  ...
  const btn = document.querySelector(
    '#panel-pbkdf2 button[onclick="pbkdf2Generate()"]',
  );
  if (btn) {
    btn.disabled = true;
    btn.textContent = "计算中…";

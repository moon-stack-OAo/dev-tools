...
function gmsmSwitchTab(tab) {
  ...
  contents.forEach((c) =>
    c.classList.toggle("active", c.id === "gmsmTab-" + tab),
  );
}

function ensureSmLoaded() {
  if (
    typeof window.sm3 === "undefined" ||
    typeof window.sm4 === "undefined" ||
    typeof window.sm2 === "undefined"
  ) {
    toast("国密库未加载");
    return false;
  }
...
function gmsmSm4Encrypt() {
  ...
  if (!pwd || pwd.length !== 16) {
    out.textContent = "SM4 密钥必须为 16 个字符（128 bit）";
    out.className = "output-box error";
    return;
  }
  try {
    ...
    if (iv) {
      if (iv.length !== 16) {
        out.textContent = "IV 必须为 16 个字符（128 bit）";
        out.className = "output-box error";
        return;
      }
      opts.iv = iv;

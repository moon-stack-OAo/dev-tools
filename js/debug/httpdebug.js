...
function httpSwitchTab(el, name) {
  ...
  root
    .querySelectorAll(".api-tab-panel")
...
function httpSetFormat(fmt) {
  _httpFormat = fmt;
  document
    .getElementById("httpFmtMulti")
    .classList.toggle("active", fmt === "multi");
  document
    .getElementById("httpFmtSingle")
    .classList.toggle("active", fmt === "single");
  const out = document.getElementById("httpCurlOutput");
  if (out && out.dataset.cmd)
    out.textContent = httpFormatOutput(out.dataset.cmd);
}
...
function httpCollectAuthHeaders() {
  ...
  if (v === "bearer") {
    ...
    const loc =
      (document.getElementById("httpAuthApiLoc") || {}).value || "header";
    if (name && val) {
      if (loc === "header") {
        out.push([name, val]);
      } else if (loc === "query") {
...
function httpBuildRequestConfig() {
  ...
  authOnlyHeaders.forEach((ah) => {
    const idx = mergedHeaders.findIndex(
      (h) => h[0].toLowerCase() === ah[0].toLowerCase(),
    );
...
function httpGenerate() {
  ...
  document.getElementById("httpStatUrl").textContent =
    cfg.url.length > 60 ? cfg.url.slice(0, 57) + "..." : cfg.url;
  document.getElementById("httpStatHeaders").textContent = cfg.headers.length;
  document.getElementById("httpStatBody").textContent = cfg.body
    ? cfg.body.length
    : 0;
...
function httpExtractFilename(disposition, url, contentType) {
  const m =
    disposition && disposition.match(/filename\*?=(?:UTF-8'')?["']?([^"';]+)/i);
  if (m) {
    try {
...
function httpSend() {
  ...
  document
    .querySelectorAll(".http-side-panel")
    .forEach((p) => p.classList.remove("active"));
  ...
  ensureProxy
    ...
    .then(async (resp) => {
      ...
      _httpLastFilename = httpExtractFilename(
        disposition,
        cfg.url,
        contentType,
      );
      ...
      if (isText && size < 5 * 1024 * 1024) {
        ...
        if (previewable && contentType.startsWith("image/"))
          ...
          iconCls = "bi-file-earmark-pdf";
        else if (contentType.startsWith("video/")) iconCls = "bi-film";
        else if (contentType.startsWith("audio/"))
          iconCls = "bi-music-note-beamed";
        else iconCls = "bi-file-earmark-zip";

        const tip = previewable
          ? "点击「打开」在新窗口预览，或「下载」保存文件"
          : "点击「下载」保存文件";
        let html =
        ...
        if (previewable && contentType.startsWith("image/")) {
          ...
          html +=
            '<div class="http-blob-preview"><img src="' +
            blobUrl +
            '" alt="preview"></div>';
...
function httpBuildErrorDiagnosis(err, cfg) {
  ...
  if (err.name === "ProxyUnavailableError" || /cors_proxy|本地 CORS 代理/i.test(msg)) {
    ...
    const LOCAL_HOST =
      /^(localhost|127\.0\.0\.1|::1|192\.168\.|10\.|172\.(1[6-9]|2[0-9]|3[0-1])\.)/i;
    const isTargetLocal = LOCAL_HOST.test(
      (cfg.url.match(/^https?:\/\/([^\/:?#]+)/i) || [])[1] || cfg.url,
    );
    const isPageLocal =
      location.protocol === "file:" || LOCAL_HOST.test(location.hostname);

    if (isHttps && isHttp) {
      causes.push("HTTPS 页面访问 HTTP 资源（混合内容被浏览器拦截）");
      solutions.push(
        "将目标服务升级到 HTTPS，或在 HTTP 服务端设置重定向到 HTTPS",
      );
    } else if (isTargetLocal && isPageLocal) {
      causes.push(
        "如果浏览器直接访问 URL 能打开，但工具 fetch 失败，常见原因：",
      );
      causes.push(
        "1) CORS 跨域拦截：服务端未返回 Access-Control-Allow-Origin（浏览器地址栏不受限，fetch 才受限）",
      );
      causes.push(
        "2) 鉴权缺失：网关需要 Cookie / Token / Referer，浏览器地址栏会自动带，fetch 不会",
      );
      causes.push(
        "3) 服务只监听 127.0.0.1，局域网其他机器访问不到（应监听 0.0.0.0）",
      );
      ...
      solutions.push(
        "curl + 响应头：curl -I " +
          cfg.url +
          " 查看 Access-Control-Allow-Origin",
      );
      solutions.push(
        "若是 CORS：让后端加响应头 Access-Control-Allow-Origin: *",
      );
      solutions.push(
        "若是鉴权：在 Auth 标签加 Cookie / Authorization，在 Headers 加 Referer/Origin",
      );
      solutions.push("按 F12 网络面板看 OPTIONS 预检 / 401 / 403 等具体状态");
    } else if (isTargetLocal && !isPageLocal) {
      causes.push(
        "当前页面部署在公网，无法访问本机 / 局域网地址（CORS + 浏览器安全策略）",
      );
      solutions.push("仅在本机 / VPN 内网使用本工具访问本地服务");
      solutions.push(
        "团队共享：部署 API 代理（ngrok / Cloudflare Tunnel / 自建 relay）",
      );
      solutions.push("或给目标服务加 HTTPS + CORS 后部署到公网");
    } else {
      causes.push("可能原因：");
      causes.push(
        "1) CORS 跨域限制 — 服务端未返回 Access-Control-Allow-Origin 响应头",
      );
      causes.push("2) 网络断开 / DNS 解析失败 / 目标主机不可达");
      causes.push("3) 协议错误（HTTPS 证书无效、TLS 握手失败）");
      solutions.push("按 F12 打开「网络」面板查看具体失败请求详情");
      solutions.push(
        "若是 CORS 错误：在目标服务端响应头添加 Access-Control-Allow-Origin: *（开发环境）",
      );
  ...
  html +=
    '<div class="http-error-title"><i class="bi ' +
    icon +
    '"></i> ' +
    title +
    "</div>";
  ...
  html +=
    '<div class="http-error-tech">原始错误: ' + escapeHtml(msg) + "</div>";
  html += "</div>";
  return html;
}
...
function httpFillSample() {
  ...
  document.getElementById("httpBodyType").value = "json";
  document.getElementById("httpBody").value = JSON.stringify(
    { name: "张三", age: 25 },
    null,
    2,
  );
...
function httpReset() {
  ...
  document.getElementById("httpBodyType").value = "none";
  document.getElementById("httpCurlInput").value = "";
  document.getElementById("httpCurlOutput").textContent =
    "点击「生成 cURL」查看结果";
...
function httpInit() {
  if (
    document.getElementById("httpHeadersList") &&
    !document.querySelector("#httpHeadersList .api-kv-row")
  ) {
    httpAddHeader("Content-Type", "application/json");
    httpAddQuery();
  }

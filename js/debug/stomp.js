...
function stompParseFrame(raw) {
  ...
  if (headerBlock) {
    headerBlock
      .split(STOMP_LF)
      .forEach((line) => stompParseLine(line, headers));
  }
...
function stompConnect() {
  ...
  stompClient.onopen = () => {
    ...
    const extra = stompParseExtraHeaders(
      document.getElementById("stompConnHeaders").value,
    );
...
function stompSend() {
  ...
  const extra = stompParseExtraHeaders(
    document.getElementById("stompSendHeaders").value,
  );
  Object.assign(headers, extra);

...
function cronSetMode(fid, mode) {
  if (mode === 0) cronState[fid] = "*";
    ...
    cronState[fid] =
      "*/" + cronGuessStep(CRON_FIELDS.find((f) => f.id === fid));
...
function cronUpdateFields() {
  CRON_FIELDS.forEach((field, fi) => {
    ...
    if (mode === 1) {
      ...
      rangeRow.querySelector(".cron-range-start").value =
        parts[0] === "*" ? field.min : parseInt(parts[0]);
      rangeRow.querySelector(".cron-range-step").value =
        parseInt(parts[1]) || cronGuessStep(field);
    }
    // 格子
...
function cronParse() {
  ...
  try {
    ...
    const parsed = fields.map((f, i) =>
      cronParseField(f, ranges[i][0], ranges[i][1]),
    );
    ...
    while (results.length < count && maxIter-- > 0) {
      ...
      if (domWild && dowWild) dayMatch = true;
        ...
        dayMatch = doms.includes(d) || dows.includes(dw);

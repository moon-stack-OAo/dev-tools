...
function formatStackTraceHtml(parsed) {
  ...
  parsed.exceptions.forEach((ex, exIndex) => {
    ...
    if (ex.message) {
      ...
      html +=
        '<span style="color:#ffd93d">' + escapeHtml(ex.message) + "</span>";
    }
    html += "</div>";
    ...
    ex.frames.forEach((frame) => {
      if (frame.omitted) {
        ...
        html +=
          '<span style="color:#4ecdc4">' + escapeHtml(frame.class) + "</span>";
        html += '<span style="color:var(--text-dim)">.</span>';
        html +=
          '<span style="color:#45b7d1;font-weight:600">' +
          escapeHtml(frame.method) +
          "</span>";
        html += '<span style="color:var(--text-dim)">(</span>';
        html +=
          '<span style="color:#96ceb4">' + escapeHtml(frame.file) + "</span>";
        if (frame.line > 0) {
          html += '<span style="color:var(--text-dim)">:</span>';
...
function stacktraceParse() {
  ...
  if (!input.trim()) {
    output.innerHTML =
      '<span style="color:var(--text-dim)">请粘贴异常堆栈</span>';
    return;
  }
  ...
  try {
    ...
    if (parsed && parsed.exceptions.length > 0) {
      ...
      output.innerHTML =
        '<span style="color:var(--text-dim)">未识别到有效的堆栈信息</span>';
    }
  } catch (e) {
    output.innerHTML =
      '<span style="color:#ff6b6b">解析失败: ' +

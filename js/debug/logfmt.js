...
function renderStackFramesHTML(frames, parentLevel) {
  ...
  const cls =
    parentLevel === "ERROR" || parentLevel === "FATAL"
      ? "logfmt-stack-error"
      : "logfmt-stack-warn";
  ...
  const body = frames
    .map(function (f) {
      ...
      if (CAUSED_BY.test(f)) {
        return (
          '<div class="logfmt-stack-line logfmt-caused">' +
          _lfEscape(f) +
          "</div>"
        );
      }
      ...
      if (ex) {
        ...
        return (
          ...
          "</span>" +
          (ex[2]
            ? '<span class="logfmt-exception-msg">' +
              _lfEscape(": " + ex[2]) +
              "</span>"
            : "") +
          "</div>"
...
function renderGroupHTML(group) {
  ...
  const entryHtml =
    '<div class="logfmt-line logfmt-level-' +
    group.entry.level +
    '">' +
...
function logfmtRender() {
  ...
  if (!input.trim()) {
    root.innerHTML =
      '<div class="logfmt-empty">点击「格式化」查看高亮结果</div>';
    return;
  }
  ...
  if (!filtered.length) {
    root.innerHTML =
      '<div class="logfmt-empty">没有匹配的行（检查级别过滤 / 搜索）</div>';
    return;
  }
...
function logfmtClear() {
  ...
  document.getElementById("logfmtResults").innerHTML =
    '<div class="logfmt-empty">点击「格式化」查看高亮结果</div>';
  const stats = document.getElementById("logfmtStats");
...
function logfmtToggleLevel(level) {
  ...
  const btn = document.querySelector(
    '.logfmt-level-btn[data-level="' + level + '"]',
  );

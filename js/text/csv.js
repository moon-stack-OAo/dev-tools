function csvFormat() {
  ...
  let html =
    '<div style="font-family:var(--font);font-size:12px;overflow:auto">';
  padded.forEach((row, i) => {
    let rowHtml =
      '<div style="display:flex;' +
      (i === 0 && hasHeader
        ? "font-weight:600;background:var(--accent-glow);border-radius:4px"
        : "") +
      '">';
    row.forEach((cell, j) => {
      rowHtml +=
        ...
        (widths[j] * 8 + 16) +
        'px;padding:3px 8px;border-right:1px solid var(--border);border-bottom:1px solid var(--border)">' +
        escapeHtml(cell) +
        "</span>";
    });
    rowHtml += "</div>";

...
function _qrdecodeShow(img, result) {
  ...
  previewImg.style.cssText =
    "max-width:200px;max-height:200px;border-radius:6px;border:1px solid var(--border);background:#fff";
  ...
  dataEl.style.cssText =
    "font-family:var(--font);font-size:13px;background:var(--bg-input);padding:8px 10px;border-radius:4px;word-break:break-all;white-space:pre-wrap;max-height:160px;overflow:auto";
  ...
  regenBtn.innerHTML =
    '<i class="bi bi-arrow-left-right"></i> 用此内容生成二维码';
  ...
  if (type === "WiFi") {
    ...
    if (wifi) {
      ...
      detail.style.cssText =
        "margin-top:8px;padding:8px;background:rgba(0,212,170,.05);border:1px solid var(--border);border-radius:4px;font-size:12px;line-height:1.7";
      detail.innerHTML =
        "<b>WiFi 信息:</b><br>网络名(SSID): " +
        escapeHtml(wifi.ssid || "-") +
        "<br>密码: " +
        escapeHtml(wifi.password || "-") +
        "<br>加密: " +
        escapeHtml(wifi.auth);
    ...
    const detail = document.createElement("div");
    detail.style.cssText =
      "margin-top:8px;padding:8px;background:rgba(167,139,250,.08);border:1px solid var(--border);border-radius:4px;font-size:12px;color:var(--accent)";
    detail.innerHTML =
      "💡 检测到 <b>otpauth</b> URI，可前往 <b>TOTP 动态令牌</b> 工具粘贴使用";
    resultBox.appendChild(detail);
  }
...
function _qrdecodeShowError(msg) {
  ...
  const meta = document.getElementById("qrdecodeMeta");
  preview.innerHTML =
    '<div style="color:var(--text-dim);font-size:12px;padding:40px 0;text-align:center">无预览</div>';
  resultBox.innerHTML = "";
...
function qrdecodeClear() {
  ...
  const meta = document.getElementById("qrdecodeMeta");
  preview.innerHTML =
    '<div style="color:var(--text-dim);font-size:12px;padding:40px 0;text-align:center">等待上传图片</div>';
  resultBox.innerHTML = "";

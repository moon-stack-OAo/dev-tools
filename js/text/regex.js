...
function regexInit() {
  ...
  REGEX_CHEAT.forEach((group) => {
    ...
    header.style.cssText =
      "font-size:13px;font-weight:600;color:var(--accent);padding:10px 0 6px;border-bottom:1px solid var(--border);margin-bottom:6px";
    header.textContent = group.cat;
    container.appendChild(header);
    const grid = document.createElement("div");
    grid.style.cssText =
      "display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:6px;margin-bottom:6px";
    group.items.forEach((item) => {
      const chip = document.createElement("div");
      chip.style.cssText =
        "display:flex;flex-direction:column;gap:2px;padding:6px 8px;border:1px solid var(--border);border-radius:6px;background:var(--bg-card);cursor:pointer;transition:border-color .12s,background .12s;font-size:12px";
      chip.title = "点击插入: " + item.t;
      chip.innerHTML =
        '<code style="color:var(--accent);font-family:var(--font);font-weight:600">' +
        escapeHtml(item.t) +
        '</code><span style="color:var(--text-dim)">' +
        escapeHtml(item.d) +
        "</span>";
      ...
      chip.addEventListener("click", () =>
        regexInsertToken(item.t, group.target === "flags"),
      );

...
function jsonToCsv(jsonInput, opts) {
  ...
  const flatRows = doFlatten
    ? data.map((row) => flattenObject(row, keySep, arrayStyle))
    : data;
...
function jsonToXlsx(jsonInput, opts) {
  ...
  const doFlatten = opts.flatten !== false;
  const arrayStyle = opts.arrayStyle || DEFAULT_ARRAY_STYLE;
  const rows = doFlatten
    ? data.map((r) => flattenObject(r, ".", arrayStyle))
    : data;
...
async function jseConvert() {
  ...
  try {
    ...
    if (src === "json" && tgt === "xlsx") {
      ...
      _jseRenderPreview(arr, _jsePreviewLimit);
      _jseSetStatus(
        "JSON → Excel 完成,共 " + arr.length + " 行,点击下载获取 .xlsx",
        false,
      );
      return;
    }
    ...
    if (src === "csv" && tgt === "xlsx") {
      ...
      _jseRenderPreview(arr, _jsePreviewLimit);
      _jseSetStatus(
        "CSV → Excel 完成,共 " + arr.length + " 行,点击下载获取 .xlsx",
        false,
      );
      return;
    }
  ...
  if (ext === "xlsx" || ext === "xls" || ext === "xlsm" || ext === "xlsb") {
    reader.onload = async () => {
      try {
        ...
        // 自动预览第一个 sheet
        const firstData = XLSX.utils.sheet_to_json(
          _jseXlsxWorkbook.Sheets[_jseXlsxWorkbook.SheetNames[0]],
          {
            defval: "",
          },
        );
        ...
        _jseSetStatus(
          "已加载 " +
            file.name +
            " · " +
            _jseXlsxWorkbook.SheetNames.length +
            " 个 Sheet",
          false,
        );
      } catch (e) {
    ...
    reader.onload = () => {
      ...
      const sep =
        ext === "tsv"
          ? "\t"
          : document.getElementById("jseSeparator").value || ",";
      // 临时设置分隔符(只影响本次预览)
...
function jseOnSheetChange() {
  ...
  if (!name) return;
  try {
    const data = XLSX.utils.sheet_to_json(_jseXlsxWorkbook.Sheets[name], {
      defval: "",
    });
    _jseRenderPreview(data, _jsePreviewLimit);
    _jseSetStatus(
      "已切换到 Sheet「" + name + "」共 " + data.length + " 行",
      false,
    );
  } catch (e) {

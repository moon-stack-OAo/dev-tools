...
function _ddExtractTableName(stmt) {
  const m = stmt.match(
    /^\s*CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?[`"[]?([A-Za-z_][\w$]*)[`"\]]?\s*\(/i,
  );
  return m ? m[1] : null;
}
...
function _ddParseColumn(line) {
  ...
  if (
    _ddFindKeyword(rest, /^AUTO_INCREMENT\b/i) >= 0 ||
    _ddFindKeyword(rest, /^AUTOINCREMENT\b/i) >= 0
  ) {
    autoInc = true;
  }
...
const _DDL_LENGTH_AWARE_TYPES = new Set([
  "VARCHAR",
  "CHAR",
  "CHARACTER VARYING",
  "CHARACTER",
...
function _ddCompareColumn(sc, dc) {
  ...
  if (_ddHasLength(sn) && _ddHasLength(dn)) {
    if (
      (sc.length == null ? null : sc.length) !==
      (dc.length == null ? null : dc.length)
    ) {
      diffs.push("length");
    }
  }
  ...
  const sDef =
    sc.default === null || sc.default === undefined ? null : String(sc.default);
  const dDef =
    dc.default === null || dc.default === undefined ? null : String(dc.default);
...
function diffSchemas(srcTables, dstTables) {
  ...
  allNames.forEach((name) => {
    ...
    changes.sort(
      (a, b) =>
        order[a.action] - order[b.action] || a.column.localeCompare(b.column),
    );
  ...
  results.sort(
    (a, b) =>
      tableOrder[a.status] - tableOrder[b.status] ||
...
function _ddiffColInfoHtml(info) {
  ...
  const bits = [];
  bits.push(
    '<span class="ddiff-info-type">' + escapeHtml(info.type) + "</span>",
  );
  ...
  if (info.autoInc)
    bits.push('<span class="ddiff-badge ddiff-badge-auto">AUTO_INC</span>');
  if (info.primaryKey)
    bits.push('<span class="ddiff-badge ddiff-badge-pk">PK</span>');
  if (info.unique)
    bits.push('<span class="ddiff-badge ddiff-badge-uk">UNIQUE</span>');
  if (info.default !== null && info.default !== undefined) {
    bits.push("DEFAULT <code>" + escapeHtml(String(info.default)) + "</code>");
  }
  if (info.comment) {
    bits.push(
      '<span class="ddiff-comment">// ' + escapeHtml(info.comment) + "</span>",
    );
  }
  return bits.join(" ");
}
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (r.status === "modified") {
      ...
      for (let j = 0; j < r.changes.length; j++) {
        const c = r.changes[j];
        const rowCls =
          c.action === "added" ? "ddiff-row-added" : "ddiff-row-removed";
        body +=
          '<tr class="' +
          rowCls +
          '">' +

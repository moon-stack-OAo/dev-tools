...
function jsonschemaSwitchTab(tab) {
  ...
  contents.forEach((c) =>
    c.classList.toggle("active", c.id === "jsonschemaTab-" + tab),
  );
}
...
function jsonschemaValidate() {
  ...
  try {
    ...
    if (valid) {
      ...
      const lines = [
        "✗ 校验失败，共 " + validate.errors.length + " 个错误：",
        "",
      ];
...
function jsonschemaLoadSample(which) {
  if (which === "valid") {
    document.getElementById("jsonschemaData").value =
      JSONSCHEMA_SAMPLE_VALID_DATA;
    document.getElementById("jsonschemaSch").value = JSON.stringify(
      JSONSCHEMA_SAMPLE_SCHEMA,
      null,
      2,
    );
    document.getElementById("jsonschemaValidResult").textContent = "";

...
(function () {
    ...
    function pmMergeMove(id, dir) {
        ...
        [mergeState.files[idx], mergeState.files[target]] = [mergeState.files[target], mergeState.files[idx]];
        pmMergeRender();
    }
    ...
    function pmMergeExport() {
        ...
        setTimeout(async () => {
            try {
                ...
                if (typeof alert === 'function') alert('合并失败：' + (err.message || err));
            } finally {
                btn.disabled = false;
    ...
    async function pmSplitPick(file) {
        ...
        try {
            ...
            if (typeof alert === 'function') alert('读取 PDF 失败：' + (err.message || err));
            ...
            try {
                ...
                if (reverse) {
                    ...
                    outs.forEach((bytes, i) => {
                        ...
                        triggerDownload(blob, `${baseName}-part${i + 1}-${timestamp()}.pdf`);
                    });
                } else {
                ...
                if (typeof alert === 'function') alert('拆分失败：' + (err.message || err));
            } finally {
                btn.disabled = false;
                btn.innerHTML = oldHtml;
            }

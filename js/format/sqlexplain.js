...
function _sqlexMysqlBlock(qb, parentSelectId) {
  ...
  for (let i = 0; i < tables.length; i++) {
    const tn =
      tables.length > 1
        ? _sqlexMysqlTableToNode(tables[i].src, selectId)
        : root;
...
function _sqlexMysqlAttachChildren(tableNode, tableCtx) {
  ...
  if (
    t.materialized_from_subquery &&
    t.materialized_from_subquery.query_block
  ) {
    const sub = _sqlexMysqlBlock(
      t.materialized_from_subquery.query_block,
      null,
    );
    if (sub) {
...
function _sqlexMysqlMakeNode(nodeType, relation, ctx) {
  ...
  if (ctx && typeof ctx === "object") {
    ...
    if (ctx.rows_examined_per_scan != null)
      n.rows = Number(ctx.rows_examined_per_scan);
    // cost_info
    if (ctx.cost_info) {
...
function parsePgExplain(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const sumM = raw.match(
      /^\s*(Planning Time|Execution Time|Trigger Runtime|JIT):\s*(.+?)\s*$/,
    );
    if (sumM) {
...
function _sqlexPgMakeNode(nodeTypeRaw, f) {
  ...
  let m = nodeTypeRaw.match(
    /^Index\s+(Only\s+)?(?:Scan|Seek)\s+using\s+(\S+)\s+on\s+(\S+)(?:\s+(\S+))?/i,
  );
  if (m) {
    key = m[2];
    relation = m[3];
    ...
    (m = nodeTypeRaw.match(
      /^(Seq|Index(?:Only)?)\s*Scan\s+on\s+(\S+)(?:\s+(\S+))?/i,
    ))
  ) {
    relation = m[2];
    alias = m[3] || null;
  } else if (
    (m = nodeTypeRaw.match(/^Bitmap\s+\w+\s+on\s+(\S+)(?:\s+(\S+))?/i))
  ) {
    relation = m[1];
    alias = m[2] || null;
...
function _sqlexPgJsonNode(node) {
  const out = {
    ...
    actualRows:
      node["Actual Rows"] != null ? Number(node["Actual Rows"]) : null,
    actualTimeStart:
      node["Actual Startup Time"] != null
        ? Number(node["Actual Startup Time"])
        : null,
    actualTimeEnd:
      node["Actual Total Time"] != null
        ? Number(node["Actual Total Time"])
        : null,
    costStart:
      node["Startup Cost"] != null ? Number(node["Startup Cost"]) : null,
...
function formatTree(tree, opts) {
  ...
  function render(node, depth) {
    ...
    const prefixChar =
      depth === 0 ? (idx + 1 === roots.length ? "└─" : "├─") : "├─";
    out.push('<div class="' + cls + '" style="margin-left:' + indent + 'px">');
    out.push('<span class="sqlex-bullet">' + prefixChar + "</span> ");
    out.push(
      '<span class="sqlex-nodetype">' + escapeHtml(line.title) + "</span>",
    );
    if (line.meta) {
      out.push(
        ' <span class="sqlex-meta">' + escapeHtml(line.meta) + "</span>",
      );
    }
    out.push("</div>");
    // 子节点
...
function _sqlexNodeLine(node) {
  ...
  if (node.accessType && node.accessType !== node.nodeType)
    meta.push("type=" + node.accessType);
  if (node.costStart != null && node.costEnd != null)
    meta.push("cost=" + node.costStart + ".." + node.costEnd);
  ...
  if (node.actualTimeEnd != null)
    meta.push("actual.time=" + node.actualTimeEnd + "ms");
  if (node.extras && node.extras.loops != null)
    meta.push("loops=" + node.extras.loops);
  if (node.extras && node.extras.queryCost != null)
    meta.push("query_cost=" + node.extras.queryCost);
...
function summarizePlan(tree) {
  ...
  function walk(n) {
    ...
    if (n.costEnd != null && n.costEnd > result.totalCost)
      result.totalCost = n.costEnd;
    if (n.costStart != null && n.costStart > result.totalCost)
      result.totalCost = n.costStart;
    if (n.extras) {
...
function sqlexRun() {
  ...
  try {
    ...
    statusEl.style.color = "var(--danger)";
    outEl.innerHTML =
      '<div class="sqlex-empty">' + escapeHtml(e.message) + "</div>";
    return;
  }
  // 渲染输出
  if (db === "mysql") {
    ...
    if (fmt === "tree") {
      ...
      if (flat.length === 0)
        out.push('<tr><td colspan="7" class="sqlex-empty">（空）</td></tr>');
      for (const n of flat) {
        const kind = _sqlexKindOf(n.nodeType, n.accessType);
        ...
        out.push("<tr>");
        out.push(
          '<td class="sqlex-cell sqlex-' +
            kind +
            '">' +
            indent +
            escapeHtml(n.nodeType) +
            "</td>",
        );
        ...
        out.push(
          "<td>" + escapeHtml(n.rows != null ? String(n.rows) : "") + "</td>",
        );
        out.push(
          "<td>" +
            escapeHtml(n.actualRows != null ? String(n.actualRows) : "") +
            "</td>",
        );
        out.push(
          "<td>" +
            escapeHtml(n.actualTimeEnd != null ? String(n.actualTimeEnd) : "") +
            "</td>",
        );
        out.push("</tr>");
      }
      ...
      outEl.innerHTML =
        '<pre class="sqlex-pre">' +
        escapeHtml(JSON.stringify(tree, null, 2)) +
        "</pre>";
    }
  }
  // 汇总
